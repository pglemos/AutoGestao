...
| Resolvido no checkpoint histórico `e609bb72` | Replay/grants RLS | O primeiro SHA falhou no histórico duplicado e o seguinte falhou na ACL; o CI histórico passou os 40 cenários, mas o resultado não prova o checkpoint vigente `d7356687` | `pgTAP RLS Matrix` `31377957069`: `SUCCESS`; checksums/reversibilidade também verdes no checkpoint histórico | Reexecutar a matriz no checkpoint vigente e manter o gate em futuras migrations; nenhum DDL foi aplicado à produção |
