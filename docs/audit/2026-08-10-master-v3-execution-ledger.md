...
- **EV-012**: Fase 4 contract audit — 3 drifts fixed by contract, all gates green. (1) T4.6 removed `--radius` from the shadcn bridge in `semantic.css` without updating the contract; restored as derivation `--radius: var(--mx-radius-lg)` (10px = 0.625rem Base44) and `tokens-contract.test.ts` now locks the derivation, `BASE44_MEASURED` reduced to `primary` only. (2) T4.8 normalized sidebar header 54→52px (4px grid) without updating metric/test/doc; aligned `SIDEBAR_METRICS.headerHeight: 52` with explanatory comment, `sidebar-contract.test.ts` (`toBe(52)`, `h-[52px]`), and `sidebar-dono.md` (lines 96, 122). (3) T4.8 typography migration left the sidebar contract on pre-migration values; synchronized `text-[9px]`/`text-[10px]`/`text-[13px]` expectations to canonical `text-caption`/`text-body-sm` (badge, sectionLabel, nestedItem, brandTitle) matching `tokens.ts`. Gates: `bun test` 2622/2622 pass across 465 files, `npm run lint` 0 violations (tsc + 15 guards + eslint).
