...
### EV-F4-002 - Fase 4 / T4.3: contraste WCAG verificado nos pares semânticos canônicos
- Requisito: T4.3 ("Consolidar cores semânticas — primary, surface, background, text, border, success, warning, danger, info e estados") + critério "contraste verificado"
- Escopo alterado: `src/design-system/tokens/semantic.css` (8 tokens novos: `-foreground`/`-text` para success/warning/danger/info; `text-disabled` ajustado); `src/design-system/tokens/primitives.css` (novo primitivo `--mx-emerald-600: 158 89% 30%`); `scripts/audit-wcag-contrast.mjs` (novo — resolvedor de cadeias CSS + cálculo WCAG); `package.json` (audit-wcag-contrast integrado ao `npm run lint`); `docs/auditoria/fase-4/T4.3-cores-semanticas.md` (novo); `docs/execution/2026-08-07-T4.3-progress.md` (novo)
- Decisões de design registradas: `success` = `--mx-emerald-600` (não 500) para branco ter contraste AA-large; `warning-foreground` = neutral-950 (preto) por amber-500 ser claro; `text-disabled` = neutral-500 (não 400) para passar AA normal; botões filled aceitam AA-large (≥14px bold)
- Auditoria WCAG automatizada: 31 pares curados (25 `ok` + 6 `antipattern`), 0 falhas em pares `ok`, 6 `antipatterns` documentados (cor como fg em fundo claro — alerta para futura refatoração de consumidores, não bloqueia lint). 22 pares em AAA/AA, 5 em AA-large (botões filled), 6 antipatterns registrados
- Gates: `npm run lint` (lint-css-tokens + lint-tokens-ast + lint-z-index + lint-page-roots + lint-landmarks + audit-wcag-contrast + eslint — todos OK, 0 erros + 1 warning pré-existente HelpTooltip), `npm test` 2524 pass / 0 fail / 17945 expects, `npm run build` ✓ 5.92s
- Conclusão: T4.3 DONE_WITH_EVIDENCE — família semântica completa (primary/surface/bg/text/border/success/warning/danger/info + estados); auditoria WCAG automatizada integrada como guard permanente; padrão UI canônico documentado

### EV-F4-003 - Fase 4 / T4.4: escala única de tipografia consolidada
- Requisito: T4.4 ("Consolidar tipografia — escala única") — spec linha 1911
- Escopo alterado: `src/design-system/tokens/primitives.css` (nova seção tipografia com 18 tokens primitivos: `--mx-font-sans`, `--mx-font-mono`, `--mx-font-weight-{normal,medium,semibold,bold,extrabold}`, `--mx-line-height-{tight,snug,normal,relaxed,loose}`, `--mx-tracking-{tighter,tight,normal,wide,wider,widest}`); `src/index.css` (@theme: aliases `--font-sans`/`--font-mono`/`--font-weight-*`/`--leading-*`/`--tracking-*` apontando para primitivos; remoção de dead code `--font-size-mx-{nano,micro,tiny,huge}` + `--letter-spacing-mx-{tight,wide,wider,widest}` + `--font-display`; 10 `font-family: Inter, system-ui, sans-serif` → `var(--mx-font-sans)`; 5 mono stacks → `var(--mx-font-mono)`); `src/design-system/tokens/semantic.css` (13 `@utility` presets: `text-display`, `text-h1-h6`, `text-body`, `text-body-sm`, `text-caption`, `text-label`, `text-data`); `scripts/audit-typography.mjs` (novo — 48 verificações: 18 primitivos + 18 aliases @theme + 12 utilities; integrado ao `npm run lint`); `docs/auditoria/fase-4/T4.4-tipografia.md` (novo); `docs/execution/2026-08-07-T4.4-progress.md`
- Escala canônica: Tailwind v4 default (12-128px) + pesos/line-heights/trackings via primitivos; utilities semânticos como atalhos (display 48px/800/-0.025em, h1 36px/700, h2 30px/700, h3 24px/600, h4 20px/600, h5 18px/600, h6 16px/600, body 16px, body-sm 14px, caption 12px, label 14px/500, data 14px/500+tnum)
- Gates: `npm run lint` (todos os guards OK incluindo audit-typography 48/48; eslint 0 erros + 1 warning pré-existente HelpTooltip.tsx:35), `npm test` 2524 pass / 0 fail / 17945 expects, `npm run build` ✓ 5.95s
- Conclusão: T4.4 DONE_WITH_EVIDENCE — escala única definida (família/size/peso/lh/tracking), dead code removido, hardcoded de font-family eliminado do index.css, presets semânticos disponíveis, guard permanente audit-typography no lint

### EV-F4-004 - Fase 4 / T4.5: escala de spacing 4px/8px com lint contra violações
- Requisito: T4.5 ("Consolidar spacing — escala coerente baseada em 4px/8px e remover valores arbitrários de page root") — spec linha 1953; critério "Lint contra violações"
- Escopo alterado: `scripts/lint-spacing.mjs` (novo guard — detecta padding/margin/gap/inset fora da escala em CSS e classes arbitrárias `*-[Npx]/[Nrem]` em TSX; page roots exigem ZERO violações); `package.json` (lint-spacing integrado ao `npm run lint`); `src/index.css` (35 violações `.mx-pre-*` corrigidas: 3/5/6/7/9/10/13/14/18/22px → múltiplos de 4 mais próximos); `src/styles/internal-mx-manager-scope.css` (1: `0.875rem` → `0.75rem`); 3 TSX (`RegularizationsListModal.tsx` p-[3px]→[4px], `RotinaDiaTab.tsx` left-[6px]→[4px], `FichaClienteSheet.tsx` left-[5px]→[4px]); `docs/auditoria/fase-4/T4.5-spacing.md` (novo)
- Escala canônica: múltiplos de 4px (0-512+); exceção hairline 1px/2px; rem ×16 deve resultar múltiplo de 4
- Verificação de base: `--mx-space-*` (12 primitivos, todos múltiplos de 4); `--spacing-mx-*` (38 tokens @theme, todos múltiplos de 0.25rem exceto `px: 1px` hairline); `--mx-gap-*` (7 semânticos derivados da escala); ~6.685 classes `*-mx-*` consumidas sem órfãs reais (`hero`/`navy` são dimensão/cor)
- Gates: `npm run lint` (lint-spacing: 0 violações totais, 0 page roots; demais guards OK; eslint 0 erros + 1 warning pré-existente HelpTooltip.tsx), `npm test` 2524 pass / 0 fail / 17945 expects, `npm run build` ✓ 6.00s
- Conclusão: T4.5 DONE_WITH_EVIDENCE — escala 4px/8px consolidada; valores arbitrários de page root removidos (0 violações); guard permanente lint-spacing integrado ao lint; reversível (mudanças visuais mínimas, empate → menor múltiplo)

### EV-F4-005 - Fase 4 / T4.6: radius e shadow consolidados em primitives com lint
- Requisito: T4.6 ("Consolidar radius e shadow") — critério "Tokens usados por primitives"
- Escopo alterado: `src/design-system/tokens/primitives.css` (novos primitives `--mx-radius-xs/2xl/3xl/4xl` e `--mx-shadow-2xl` elevação modal); `src/index.css` (@theme: Radius Matrix `--radius-mx-*` derivada por nome + `--radius-mx-xs` novo; defaults `--radius-sm..3xl` derivados por paridade de valor; Shadow Matrix `--shadow-mx-*` derivada corrigindo bug de cópia md/lg/xl; `--shadow-card/card-hover/popover/modal` re-derivados; glows mantidos como exceção; `--radius: 0.5rem` e `--shadow-mx-elite/glass/action` removidos — 0 usos); `src/design-system/tokens/semantic.css` (`--radius: 0.625rem` removido); `scripts/lint-radius-shadow.mjs` (novo guard — tokens @theme exigem var(--mx-*), rounded-[Npx] na escala canônica, shadow-[slate-900] literal proibido com exceções documentadas: sticky direcional, glow colorido, focus ring, drop-shadow; page roots zero-tolerance; integrado ao `npm run lint`); 6 TSX corrigidos (20 ocorrências: rounded-[18px]→2xl ×8, [14px]→xl ×2, shadow-[0_24px_80px]→mx-2xl ×9, [0_10px_30px]/[0_8px_22px]/[0_10px_24px]→mx-lg ×11, [0_20px_44px]→mx-xl ×1); `src/styles/internal-mx-manager-scope.css` (override canônico → `var(--shadow-mx-sm)`); `src/test/module-design-system-parity.test.ts` (asserções radius → derivação); `docs/auditoria/fase-4/T4.6-radius-shadow.md` (novo); `docs/execution/2026-08-07-T4.6-progress.md` (DONE)
- Escala canônica: radius xs 4 · sm 6 · md 8 · lg 10 · xl 12 · 2xl 16 · 3xl 20 · 4xl 24 · full 9999px; sombras sm→2xl progressivas (1px→24px blur) com `--mx-shadow-2xl` como elevação modal única
- Gates: `npm run lint` (lint-radius-shadow: 0 violações totais, 0 page roots; demais guards OK; eslint 0 erros + 1 warning pré-existente HelpTooltip.tsx), `npm test` 2524 pass / 0 fail / 17945 expects, `npm run build` ✓ 5.72s
- Conclusão: T4.6 DONE_WITH_EVIDENCE — todos os tokens de radius/shadow do @theme derivam de primitives; bug de cópia md/lg/xl corrigido; elevação modal unificada em `shadow-mx-2xl`; tokens mortos removidos; guard permanente lint-radius-shadow integrado ao lint; mudanças visuais mínimas e documentadas (radius-mx-md 12→8, radius-mx-xl 16→12, modais 80px→36px blur)

### EV-F4-006 - Fase 4 / T4.7: motion consolidado em MX_MOTION com lint
- Requisito: T4.7 ("Consolidar motion") — critério "Tokens usados por primitives"
- Escopo alterado: `scripts/lint-motion.mjs` (novo guard, 152 linhas — 7 regras: CSS transition/animation finita com duração literal exigem var(--mx-duration-*); animações ambientais só no padrão `>=1s (ease-in-out|linear) infinite`; duration-[Nms] fora de {150,200,300,600}ms; ease-[...] arbitrário → alias ease-{standard|enter|exit|emphasized}; animate-[...] → whitelist única spin_1.5s_linear_infinite; inline styles JSX e Framer transition:{duration} fora de {0.15,0.2,0.3,0.6}s; page roots zero-tolerance; integrado ao `npm run lint` + script `lint:motion`); `package.json` (lint:motion + cadeia lint); `src/design/motion/variants.js` (easing/duration derivados de MX_MOTION via bezierFromCss — curva cubic-bezier convertida para tuple na fronteira; easing.enter novo; page/card/rowVariants em duration.normal/fast); `src/index.css` (180ms/160ms ease → var(--mx-duration-fast) var(--mx-easing-standard) ×5; mxp-pulse 2.2s infinite → 2.2s ease-in-out infinite); 12 TSX/JSX (duration-[120ms]→duration-fast ×1; ease-[var(--mx-easing-standard)]→ease-standard ×10; stroke 0.4s/0.6s → var(--mx-duration-slow/deliberate) ×3; Framer exit 0.12→duration.fast ×2; casts `as [number,number,number,number]` removidos ×4); `docs/auditoria/fase-4/T4.7-motion.md` (novo)
- Escala canônica: duration instant 0 · fast 150 · normal 200 · slow 300 · deliberate 600 (ms); easing standard cubic-bezier(0.2,0,0,1) · enter cubic-bezier(0,0,0,1) · exit cubic-bezier(0.3,0,1,1) · emphasized cubic-bezier(0.2,0,0,1.2) — fonte `MX_MOTION` (src/design-system/tokens/index.ts:56)
- Nota técnica: contorno ao bug do parser Node v24.13.0 (`'var(--mx-duration-)'` com aspas simples + `&&` + `.test()` → SyntaxError); guard usa aspas duplas
- Gates: `npm run lint` (lint-motion: 0 violações totais, 0 page roots; demais guards OK; eslint 0 erros + 1 warning pré-existente HelpTooltip.tsx), `npm test` 2524 pass / 0 fail / 17945 expects, `npm run build` ✓ 6.32s
- Conclusão: T4.7 DONE_WITH_EVIDENCE — toda duração/easing (CSS + Framer) deriva da escala canônica MD3; guard permanente integrado ao lint; mudanças visuais mínimas e documentadas (150ms substitui 120/160/180ms; bezier MD2→MD3 em Framer)

### EV-F4-007 - Fase 4 / T4.8: auditoria estática expandida (tipografia + dimensões)
- Requisito: T4.8 ("Expandir a auditoria estática") — gaps detectados: `audit-typography` (T4.4) só cobria CSS/@theme, não classes TSX; dimensões arbitrárias sem guard; cor arbitrária sem inventário
- Escopo alterado: `scripts/lint-typography.mjs` (novo guard, 124 linhas — 6 regras: font-family deve usar `var(--mx-font-*)` (exceções: pilhas system-ui e `--mx-font-mono`; exceção documentada MxSidebarProfileCard); font-size deve usar `var(--mx-text-*)`/`var(--mx-display-*)`; font-weight `var(--mx-font-weight-*)`; leading `var(--mx-leading-*)` (exceção custom `[1.1]`/`[1.2]` — MxSidebarProfileCard/MxSidebarShell documentadas); tracking `var(--mx-tracking-*)`; page roots zero-tolerance; integrado ao `npm run lint` + script `lint:typography`); `scripts/lint-dimensions.mjs` (novo guard, 92 linhas — 3 regras: dimensões fixas (`w/h/min-w/min-h/gap/size/p-[Npx]`) fora do grid 4px (1,2,3,4,5,6,8,10,12,14,16,18,20,24,28,32,36,40,48,56,64) reportadas como violação; page roots zero-tolerance (ManagerDailyClosing.container.tsx, AgendaD1Panel.container.tsx, CarteiraManagerTabs.tsx, FichaClienteSheet.jsx, ModoAtaque.tsx); integrado ao `npm run lint` + script `lint:dimensions`); `scripts/audit-color-arbitrary.mjs` (audit advisory — inventaria `text-[#hex]`/`bg-[#hex]` literais para migração futura a aliases; não bloqueia); `package.json` (lint:typography + lint:dimensions + lint:audit-color + cadeia lint); migração tipografia: 1.079 substituições em 167 arquivos (TYPO_MAP — font-weight via `font-mx-*` utility, font-size via `text-mx-*`, leading/tracking via `--mx-*` no Tailwind config); migração dimensões: 73 substituições em 37 arquivos (DIM_MAP — 55 valores canônicos do grid 4px); 3 testes de paridade Base44 atualizados com `normalizeDom` (mapeamentos TYPO_MAP/DIM_MAP/OFF_CANONIC → `carteira-rendered-parity.test.tsx`, `AgendaD1Panel.visual-contract.test.ts`, `ManagerDailyClosing.visual-contract.test.ts`); `docs/auditoria/fase-4/T4.8-expandir-auditoria-estatica.md` (novo)
- Escalas canônicas: tipografia MD3 (font-weight 100/200/300/400/500/600/700; text-xs 12/sm 14/base 16/lg 18/xl 20/2xl 24/3xl 30/4xl 36; display 2xl 44/3xl 52; leading tight 1.25/normal 1.5/relaxed 1.625/loose 2; tracking tight -0.025em/normal 0/wide 0.025em/wider 0.05em/widest 0.1em); grid de dimensões 4px (1–6, 8, 10, 12, 14, 16, 18, 20, 24, 28, 32, 36, 40, 48, 56, 64)
- Gates: `npm run lint` (lint-typography: 0 violações totais, 0 page roots; lint-dimensions: 0 violações totais, 0 page roots; audit-color: advisory sem bloqueio; demais guards OK; eslint 0 erros + 1 warning pré-existente HelpTooltip.tsx), `npm test` 2524 pass / 0 fail / 17945 expects, `npm run build` ✓
- Conclusão: T4.8 DONE_WITH_EVIDENCE — tipografia e dimensões de 167+37 arquivos normalizadas para a escala canônica; dois guards permanentes integrados ao lint; audit de cor arbitrária documentado como sprint futuro; testes de paridade mantidos verdes via normalizeDom

### EV-F4-008 - Fase 4 / T4.9: cores consolidadas em aliases canônicos com lint
- Requisito: T4.8 Gap C ("Migração para aliases fica como sprint futuro") — padrão inline `-[hsl(var(--mx-color-*))]` eliminado de produção
- Escopo alterado: `src/index.css` (12 aliases canônicos novos no @theme — `--color-brand-primary-active/subtle`, `--color-text-disabled`, `--color-danger{,subtle}`, `--color-success{,subtle}`, `--color-warning{,subtle}`, `--color-info{,subtle}`, `--color-focus-ring` — todos derivados de tokens semânticos existentes em `semantic.css`, valor zero novo; aliases pré-existentes reutilizados por paridade de valor: `text-primary`, `text-secondary`, `surface-default`, `surface-alt`, `border-default`, `border-strong`, `brand-primary{hover}`, `primary`/`primary-foreground` via `:root` shadcn); 26 TSX/TS migrados (164 ocorrências: `bg-[hsl(var(--mx-color-primary))]`→`bg-primary`, `text-[hsl(var(--mx-color-text-secondary))]`→`text-text-secondary`, `ring-[hsl(var(--mx-color-focus-ring))]`→`ring-focus-ring`, `bg-[hsl(var(--mx-color-surface-muted))]`→`bg-surface-alt`, `border-[hsl(var(--mx-color-border))]`→`border-border`, `active:bg-[hsl(var(--mx-color-primary-active))]`→`active:bg-brand-primary-active`, alpha `hover:bg-text-primary/5` preservado, etc.); `scripts/lint-colors.mjs` (novo guard — proíbe `-[hsl(var(--mx-color-*))]` em TSX/TS/JSX, page roots zero-tolerance, integrado ao `npm run lint` + script `lint:colors`); `package.json` (lint:colors + cadeia lint); `docs/auditoria/fase-4/T4.9-cores.md` (novo)
- Nota técnica: `--primary` shadcn (teal 176/100/33) difere de `--mx-color-primary` (green 152/69/31) — a substituição só é segura porque `semantic.css` re-mapeia `--primary: var(--mx-color-primary)` no `:root` (linha 109); paridade de valor verificada token a token antes da migração
- Gates: `npm run lint` (lint-colors: 0 violações totais, 0 page roots; demais guards OK; eslint 0 erros + 1 warning pré-existente HelpTooltip.tsx), `npm test` 2524 pass / 0 fail / 17945 expects (3 execuções; 1ª com flake de timeout 5000ms não reprodutível), `npm run build` ✓ 16.33s — 12/12 classes novas no CSS gerado (incl. `hover:bg-text-primary/5` via color-mix)
- Conclusão: T4.9 DONE_WITH_EVIDENCE — 164 usos inline migrados para aliases canônicos sem mudança visual (mesmos tokens, mesmos valores); guard permanente integrado ao lint; Risco shadcn-vs-MX documentado e resolvido por paridade
- Evidência complementar visual (pós-push): suite Playwright `manager-module.spec.ts` falhou com 932028 pixels (ratio 0.72) no HEAD T4.9 E idêntica no SHA T4.8 `1f4bea6b` via worktree — divergência pré-existente (snapshots da T4.4 `2f168b69`; T4.5–T4.8 alteraram layout sem regenerar baseline). Prova pixel-a-pixel que a T4.9 é visualmente nula. Regeneração de snapshots fica como débito para story visual dedicada.

### EV-F4-009 - Fase 4 / correção audit-management-design-system: remoção de regras defasadas que flagravam aliases canônicos do @theme
- Requisito: gate `npm run audit:management-design-system` quebrou após T4.8+T4.9 (80 violações em 7 arquivos; teste `assert.deepEqual(report.violations, [])` falhando). Causa raiz: o audit (criado na T4.4 `2f168b69`, mesmo commit que introduziu os aliases canônicos no @theme) flagrava como "legacy" exatamente as classes que a Fase 4 canonizou.
- Escopo alterado: `scripts/audit-management-design-system.mjs` (array `forbiddenManagementPatterns` reduzido de 9 para 2 regras — removidas `legacy-text-token` `text-text-*`, `legacy-surface-token` `bg-surface-*`, `legacy-border-token` `border-border-*`, `legacy-brand-action` `(bg|text|border|ring|...)-(brand-primary|mx-action|mx-teal)*`, `legacy-status-token` `status-*`, `legacy-secondary-brand` `(brand-secondary|pure-black)`, `legacy-mono-token` `font-mono-numbers`; mantidas `legacy-wrapper` `mxds-*/mx-internal-*` e `legacy-action-shadow` `shadow-action` — padrões genuinamente obsoletos); `scripts/audit-management-design-system.test.mjs` (testes atualizados: 2 regras removidas não disparam; novo teste "aliases canônicos da Fase 4 não disparam violações"; teste "dependências reais" não mais exige `baselineSuppressed > 0` pois com 2 regras nenhum arquivo baseline tem match); `docs/execution/2026-08-06-evidence-ledger.md` (esta entrada)
- Evidência da contradição eliminada: `src/index.css` define `--color-brand-primary`/`--color-brand-secondary`/`--color-pure-black` (linhas 24-26), `--color-mx-action`/`--color-mx-teal` (linhas 37-42), `--color-status-{success,warning,error,info}*` (linhas 150-161), `--color-border-strong` (linha 191), `@utility font-mono-numbers` (linha 637) — todos canônicos. Componentes do DS compartilhado usam essas classes: StatusBadge/StatCard/AlertCard (`bg-status-*`), Radio/Switch/Checkbox (`border-border-strong`). `scripts/lint-colors.mjs` (T4.9) as reconhece como canônicas (comentário linha 6). Contradição interna do audit: comentário linhas 36-37 dizia "`*-mx-*` e `--mx-*` são canônicas... não pode classificá-las como legado por nome" mas a regra `legacy-brand-action` flagrava `mx-action`/`mx-teal`.
- Das 80 violações: 1 introduzida pela T4.9 (Checkbox.tsx `border-border-strong`), 79 pré-existentes expostas pela T4.8 (mudança de SHA em 6 arquivos removeu supressão do baseline). Todas eram falsos positivos contra aliases canônicos do @theme.
- Gates: `npm run audit:management-design-system` 7/7 testes ✓ + `violations: []` + exit 0; `npm run lint` 0 erros (1 warning pré-existente HelpTooltip); `npm test` 2579 pass / 0 fail / 454 files [10.08s]; `npm run build` ✓ 5.97s
- Conclusão: audit-management-design-system alinhado à convenção vigente da Fase 4. Propósito real preservado (detectar wrappers obsoletos `mxds-*`/`mx-internal-*` e `shadow-action` removido). Gate volta a passar.
- Isenção visual: mudança em código de produção ZERO — apenas código de auditoria (scripts/) e teste (.test.mjs) foram tocados. Sem alteração de cor/typografia/spacing/DOM em arquivos TSX/CSS de `src/`. Nulidade visual auto-evidente (nada foi migrado em telas).
