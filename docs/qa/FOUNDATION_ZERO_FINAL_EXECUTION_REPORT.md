...
- Smoke/screenshots de produção: **SKIPPED (ambiental)** — Chromium não alcança `supabase.co` no sandbox (login retorna "Não foi possível conectar ao servidor de autenticação"; `curl` alcança HTTP 401). Plano CI em `docs/execution/release-checklist-ak.md`. Evidência alternativa: FASE AF 464 PASS + bypass dono/gerente/vendedor.
