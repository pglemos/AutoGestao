...
| Edge `google-calendar-sync` | Data Engineering | autenticação interna própria implementada localmente; secret remoto presente | fallback admin no cliente; Bearer legado removido | `GOOGLE_CALENDAR_SYNC_ADMIN_TOKEN` + sessão validada manualmente | 401 sem auth, token errado rejeitado e sync incremental | publicar com `verify_jwt=false` e smoke | IMPLEMENTADO LOCAL; DEPLOY PENDENTE |
...
| Edge `google-meet-ata` | Data Engineering | cron local migrado para segredo dedicado do Vault | legado removido do transporte local | `MX_CRON_SECRET` / `mx-google-meet-ata-cron-secret` | gerar/reler ata autorizada e teste negativo de auth | publicar e comprovar cron sem JWT legado | IMPLEMENTADO LOCAL; DEPLOY PENDENTE |
| Edge `google-oauth-handler` | Data Engineering | chamada interna migrada localmente para token dedicado | fallback admin somente no cliente | `GOOGLE_CALENDAR_SYNC_ADMIN_TOKEN` | callback OAuth, token cifrado e teste negativo de auth | publicar e comprovar sync sem JWT legado | IMPLEMENTADO LOCAL; DEPLOY PENDENTE |
