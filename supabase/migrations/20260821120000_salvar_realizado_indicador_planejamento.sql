-- "Metas e Realizados" só gravava a Meta. A coluna `realizado` existe desde
-- 20260527180000 (com `source`/`source_ref` prontos para auditoria), a tela
-- lê `realizado`, mas nenhuma migration, RPC ou job jamais escreveu nela —
-- o resultado real de um indicador estratégico não tinha como ser registrado
-- em lugar nenhum do sistema.
--
-- Ao contrário da meta (imutável depois de publicada — Article do plano),
-- o realizado precisa ser editável mesmo com o ciclo publicado: é assim que
-- o resultado do mês é lançado contra o alvo já fechado. Por isso esta RPC
-- não repete o bloqueio `status = 'publicado'` de salvar_metas_indicador_planejamento;
-- só exige um ciclo vigente (não revisado) para atribuir a linha a uma versão.
--
-- Limitação conhecida herdada do desenho da tabela: `source`/`source_ref` são
-- por linha (mês), não por campo. Editar meta e realizado do mesmo mês em
-- chamadas separadas faz a última gravação também redefinir `source`, mesmo
-- quando só um dos dois campos mudou. Aceitável por ora porque cada RPC grava
-- sua própria origem (meta sempre 'manual'; realizado conforme p_source) e
-- source_ref.operation distingue qual delas escreveu por último; separar
-- a origem por campo exigiria duas colunas novas — fica para quando o
-- histórico de fato precisar diferenciar as duas proveniências no mesmo mês.

BEGIN;

ALTER TABLE public.historico_valores_indicadores_planejamento
  ADD COLUMN IF NOT EXISTS field text NOT NULL DEFAULT 'meta'
  CHECK (field IN ('meta', 'realizado'));

COMMENT ON COLUMN public.historico_valores_indicadores_planejamento.field IS
  'Qual coluna este snapshot descreve: meta ou realizado.';

CREATE INDEX IF NOT EXISTS idx_hist_valores_planejamento_field
  ON public.historico_valores_indicadores_planejamento(field, loja_id, indicator_code, year, created_at DESC);
