-- Migration: 20260807210000_allow_sellers_view_store_team_ranking.sql
-- Permite que vendedores visualizem todos os vendedores da sua própria loja no ranking da unidade.
-- 1. Atualiza a RPC vendedor_performance_oficial para permitir que um vendedor consulte o ranking completo da sua loja (p_store_id).
-- 2. Atualiza a política vendedores_loja_select para que integrantes da mesma loja possam visualizar os vínculos ativos da loja.
-- 3. Atualiza public.pode_ver_usuario para permitir que integrantes da mesma loja visualizem dados básicos de perfil (usuarios) da equipe.

-- 1. Atualiza RPC vendedor_performance_oficial
...
-- 2. Atualiza política vendedores_loja_select
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM public.vinculos_loja meu
       WHERE meu.store_id = vendedores_loja.store_id
         AND meu.user_id = auth.uid()
         AND coalesce(meu.is_active, true)
    )
  );

-- 3. Atualiza public.pode_ver_usuario
  ...
  SELECT
    ...
    OR EXISTS (
      ...
      WHERE meu.user_id = uid
        ...
        AND coalesce(alvo.is_active, true)
    )
$$;
