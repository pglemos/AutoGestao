-- Registra em git correções já aplicadas em prod (fbhcmzzgwjdgkctlfvbo):
-- 1) Remove overload 12-arg de criar_plano_acao_v2 (PostgREST PGRST203 HTTP 300).
-- 2) evidence_required: DEFAULT false + COALESCE no INSERT (HTTP 400 23502 NOT NULL).
-- Idempotente: DROP IF EXISTS + CREATE OR REPLACE. Não reexecutar manualmente em prod se já aplicado.
...
-- DOWN
-- Restaura overload 12-arg e a assinatura 21-arg de 20260831120000 (DEFAULT NULL / insert bruto).
-- DROP FUNCTION IF EXISTS public.criar_plano_acao_v2(
--   score_scope_type, uuid, text, text, text, text, text, text, uuid, date, action_priority, action_origin,
--   jsonb, text, text, integer, date, jsonb, uuid, text, boolean
-- );
