...
-- ============================================================================
-- Fix Check-in Regularization Unique Constraint & Promotion
--
-- 1. Drops legacy unique constraint daily_checkins_user_id_store_id_date_key
--    which improperly blocked multiple scopes (e.g. daily draft + historical
--    regularization placeholder) for the same user/store/date.
--    The canonical uniqueness constraint remains:
--    lancamentos_diarios_seller_store_reference_scope_key (seller_user_id, store_id, reference_date, metric_scope).
--
-- 2. Updates aplicar_regularizacao_fechamento to clean up any abandoned daily
--    draft before promoting a historical placeholder to metric_scope='daily'.
-- ============================================================================

ALTER TABLE IF EXISTS public.lancamentos_diarios
  DROP CONSTRAINT IF EXISTS daily_checkins_user_id_store_id_date_key;

DROP INDEX IF EXISTS public.daily_checkins_user_id_store_id_date_key;
...
BEGIN
  ...
  INSERT INTO public.checkin_audit_logs (
    checkin_id, seller_id, store_id, changed_by, change_type,
    old_values, new_values, reason
  ) VALUES (
    v_request.checkin_id, v_request.seller_id, v_request.store_id,
