-- Atualiza vendedor_performance_oficial para considerar metas individuais
-- da tabela public.metas quando regras_metas_loja.individual_goal_mode = 'custom'.
--
-- A CTE store_rules ganha o campo individual_goal_mode, e a meta do vendedor
-- passa a ser:
--   is_venda_loja → 0
--   custom + metas table → target da metas (se existir, senão rateio)
--   even/proportional/null → monthly_goal / seller_count (rateio)
--
-- Cópia integral da função anterior com essas alterações.
  ...
  SELECT s.seller_user_id, s.seller_name, s.store_id, s.store_name,
         ...
         CASE
           ...
           WHEN sr.individual_goal_mode = 'custom' THEN
             coalesce(
               (SELECT m.target FROM public.metas m
                WHERE m.user_id = s.seller_user_id
                  AND m.store_id = s.store_id
                  AND m.month = EXTRACT(MONTH FROM p_start_date)
                  AND m.year = EXTRACT(YEAR FROM p_start_date)),
               sr.monthly_goal / sr.seller_count,
               0
             )
           ...
           WHEN sr.individual_goal_mode = 'custom' THEN
             CASE
               ...
               THEN round(coalesce(sa.vendas, 0)::numeric / (
                 SELECT coalesce(m.target, sr.monthly_goal / sr.seller_count, 0)
                 FROM public.metas m
                 WHERE m.user_id = s.seller_user_id
                   AND m.store_id = s.store_id
                   AND m.month = EXTRACT(MONTH FROM p_start_date)
             ...
             END
           ELSE
             CASE WHEN sr.monthly_goal / sr.seller_count > 0
