-- Migration: 20260817163000_fix_store_target_plan_active_seller_scope.sql
--
-- Corrige a validação de escopo em `consolidate_store_target_plan`:
-- Vendedores ativos na loja (`is_active = true`) devem ter permissão de
-- consolidar o plano de metas da sua loja mesmo ao registrar vendas com
-- data retroativa (ex: início do mês ou data anterior ao started_at de cadastro).
-- Vendedores inativos mantêm a validação histórica por janela de atuação.
