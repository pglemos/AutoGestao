-- DOWN — restaura os códigos originais do backfill e a função anterior.
--
-- O backup técnico foi criado pela migration UP e permanece protegido até o
-- rollback. Se ele não existir, interromper em vez de inventar códigos.

BEGIN;

DO $$
BEGIN
  IF to_regclass('public.migration_backup_planos_acao_codigo_20260830140000') IS NULL THEN
    RAISE EXCEPTION 'Backup técnico dos códigos não encontrado; use PITR ou snapshot anterior.';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM public.migration_backup_planos_acao_codigo_20260830140000 backup
    JOIN public.planos_acao other ON other.codigo = backup.codigo_original
    WHERE other.id <> backup.plan_id
  ) THEN
    RAISE EXCEPTION 'Rollback dos códigos abortado: existe colisão com código original.';
  END IF;

  UPDATE public.planos_acao plan
  SET codigo = backup.codigo_original
  FROM public.migration_backup_planos_acao_codigo_20260830140000 backup
  WHERE backup.plan_id = plan.id;
END;
$$;

DROP TABLE public.migration_backup_planos_acao_codigo_20260830140000;
