...
describe('páginas de planejamento do módulo interno MX', () => {
  ...
  test('abre Plano Estratégico pelos planos do cliente e honra mode=catalogo', () => {
    ...
    expect(read('src/features/admin-mx/AdminClienteDetalhePage.tsx')).toMatch(/setPlanningTab\(['"]estrategico['"]\)/)
    expect(read('src/features/admin-mx/AdminClienteDetalhePage.tsx')).toMatch(/setTab\(['"]planejamento['"]\)/)
