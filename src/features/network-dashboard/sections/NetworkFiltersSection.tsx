  ...
  return (
    <MxToolbar aria-label="Filtros do cockpit operacional">
      ...
      <MxField label="Buscar loja" className="min-w-[220px] flex-1">
        <div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} aria-hidden="true" /><Input aria-label="Buscar loja por nome" value={props.search} onChange={event => props.onSearch(event.target.value)} className="pl-9" placeholder="Ex.: nome da loja" /></div>
      </MxField>
      <MxField label="Situação" className="min-w-[168px]">
        <select aria-label="Filtrar lojas por situação" className="h-10 rounded-xl border border-border bg-white px-3 text-sm" value={props.status} onChange={event => props.onStatus(event.target.value as NetworkStatusFilter)}>
      ...
      <MxField label="Período" className="min-w-[168px]">
        <select aria-label="Filtrar lojas por período" className="h-10 rounded-xl border border-border bg-white px-3 text-sm" value={props.timeframe} onChange={event => props.onTimeframe(event.target.value as NetworkTimeframe)}>
      ...
      {props.timeframe === 'personalizada' ? (
        <>
          <MxField label="Início"><Input aria-label="Data inicial do período" type="date" value={props.customRange.start} onChange={event => props.onCustomRange({ ...props.customRange, start: event.target.value })} /></MxField>
          <MxField label="Fim"><Input aria-label="Data final do período" type="date" value={props.customRange.end} onChange={event => props.onCustomRange({ ...props.customRange, end: event.target.value })} /></MxField>
        </>
      ) : null}
