    ...
    <ReportPageShell
      ...
      filters={(
        <MxToolbar>
          ...
          <span className="text-xs text-muted-foreground">{state.lastUpdatedAt ? `Atualizado às ${state.lastUpdatedAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : 'Aguardando atualização'}</span>
        </MxToolbar>
      )}
      metrics={(
        <MxMetricGrid>
