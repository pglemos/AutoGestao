          ...
          return (
            <div key={mes.mes || i} className="flex items-center gap-3">
              ...
              <div className="flex-1 flex items-center gap-2">
                <div className="flex-1 rounded-full h-2.5" style={{ background: 'var(--color-muted)' }}>
                  <div
                    className="h-2.5 rounded-full transition-all duration-700"
                    style={{
                      width: `${barWidth}%`,
                      background: mes.isAtual
                        ? 'linear-gradient(90deg, var(--color-status-info), var(--color-status-info))'
                        : isMelhor
                          ? 'linear-gradient(90deg, var(--color-status-success), var(--color-status-success))'
                          : 'linear-gradient(90deg, var(--color-status-success), var(--color-status-success))',
                      boxShadow: isMelhor ? '0 0 6px hsl(var(--mx-status-success) / 0.4)' : 'none',
                    }}
                  />
                </div>
                {isMelhor && <Trophy className="w-3.5 h-3.5 text-status-warning-text flex-shrink-0" />}
              </div>
              <span className="text-sm font-bold tabular-nums w-24 text-right flex-shrink-0" style={{ color: mes.isAtual ? 'var(--color-status-info-text)' : isMelhor ? 'var(--color-status-success-text)' : 'var(--color-text-secondary)' }}>
