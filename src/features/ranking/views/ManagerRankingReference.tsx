          ...
          <div className="flex gap-1">
            {([['classificacao', 'Classificação', ListOrdered], ['comparativo', 'Comparar vendedores', BarChart3]] as const).map(([key, label, Icon]) => (
              <button
                ...
                onClick={() => setView(key)}
                className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-medium transition-all ${view === key ? 'bg-brand-primary text-white shadow-sm' : 'text-muted-foreground hover:bg-gray-50'}`}
              >
                <Icon size={15} /> {label}
              </button>
            ))}
...
function RankingRow({ seller, index }: { seller: RankedVendedor; index: number }) {
  ...
  const rowTone = index === 0 ? 'border-l-4 border-status-warning/50 bg-status-warning-surface' : index === 1 ? 'border-l-4 border-border-strong bg-gray-50' : index === 2 ? 'border-l-4 border-status-warning bg-status-warning-surface' : ''
  const positionTone = index === 0 ? 'bg-status-warning/50 text-status-warning-foreground' : index === 1 ? 'bg-gray-400 text-status-warning-foreground' : index === 2 ? 'bg-status-warning text-status-warning-foreground' : 'bg-gray-100 text-muted-foreground'
  const barTone = attainment === null ? 'bg-gray-300' : attainment >= 80 ? 'bg-status-success' : attainment >= 50 ? 'bg-status-warning' : 'bg-status-error'
  ...
  return <tr className={rowTone}><td className="px-4 py-3"><span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${positionTone}`}>{index + 1}</span></td><td className="px-4 py-3 font-semibold text-foreground">{seller.nome}</td><td className="px-4 py-3 text-foreground">{seller.vendas}</td><td className="px-4 py-3 text-foreground">{seller.meta || '—'}</td><td className="px-4 py-3"><div className="flex min-w-[88px] items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-100"><div className={`h-full rounded-full ${barTone}`} style={{ width: `${Math.min(attainment ?? 0, 100)}%` }} /></div><span className={`text-xs font-medium ${textTone}`}>{attainment === null ? '—' : `${attainment}%`}</span></div></td><td className="px-4 py-3 text-foreground">{seller.agendamentos}</td><td className="px-4 py-3 text-foreground">{seller.conversao}%</td><td className="px-4 py-3 text-foreground">{seller.rotina === null ? '—' : `${seller.rotina}%`}</td><td className="px-4 py-3 font-bold text-status-success-text">{seller.pontuacao ?? '—'}</td></tr>
}
