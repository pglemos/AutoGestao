...
function RankingRow({ seller, index }: { seller: RankedVendedor; index: number }) {
  ...
  return <tr className={rowTone}><td className="px-4 py-3"><span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${positionTone}`}>{index + 1}</span></td><td className="px-4 py-3 font-semibold text-gray-800">{seller.nome}</td><td className="px-4 py-3 text-gray-700">{seller.vendas}</td><td className="px-4 py-3 text-gray-700">{seller.meta || '—'}</td><td className="px-4 py-3"><div className="flex min-w-[88px] items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-100"><div className={`h-full rounded-full ${barTone}`} style={{ width: `${Math.min(attainment ?? 0, 100)}%` }} /></div><span className={`text-xs font-medium ${textTone}`}>{attainment === null ? '—' : `${attainment}%`}</span></div></td><td className="px-4 py-3 text-gray-700">{seller.agendamentos}</td><td className="px-4 py-3 text-gray-700">{seller.conversao}%</td><td className="px-4 py-3 text-gray-700">{seller.rotina === null ? '—' : `${seller.rotina}%`}</td><td className="px-4 py-3 font-bold text-emerald-700">{seller.pontuacao ?? '—'}</td></tr>
}
