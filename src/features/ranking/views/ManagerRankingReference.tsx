    ...
    <PageCanvas as="div" width="dashboard" bottomClearance="navigation" className="flex flex-col gap-5">
        <header className="rounded-2xl border border-border-subtle bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-2"><input id="manager-ranking-reference-month" name="referenceMonth" type="month" value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)} aria-label="Mês do ranking" className="rounded-xl border border-border px-3 py-2 text-sm" />{RANKING_PERIODOS.map(periodo => <button key={periodo} type="button" onClick={() => data.setPeriodo(periodo)} aria-pressed={data.periodo === periodo} className={`rounded-xl border px-3 py-2 text-sm transition-colors ${data.periodo === periodo ? 'border-emerald-600 bg-emerald-600 font-semibold text-white' : 'border-border bg-white text-muted-foreground hover:bg-gray-50'}`}>{periodo}</button>)}{data.unidades.length > 1 && <select id="manager-ranking-store" name="store" value={data.unidade} onChange={(event) => data.setUnidade(event.target.value)} aria-label="Unidade do ranking" className="rounded-xl border border-border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500"><option value="todas">Todas as unidades</option>{data.unidades.map(unidade => <option key={unidade} value={unidade}>{unidade}</option>)}</select>}<select id="manager-ranking-criterion" name="criterion" value={criterion} onChange={(event) => setCriterion(event.target.value as Criterion)} aria-label="Critério do ranking" className="rounded-xl border border-border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500"><option value="geral">Pontuação geral</option><option value="vendas">Vendas</option><option value="conversao">Conversão</option><option value="rotina">Rotina</option></select><button type="button" onClick={() => void data.handleRefresh()} aria-label="Atualizar ranking" className="grid h-10 w-10 place-items-center rounded-xl border border-border text-muted-foreground hover:bg-gray-50"><RefreshCw size={16} className={data.isRefetching ? 'animate-spin' : ''} /></button></div>
          </div>
        </header>
          ...
          <div className="flex gap-1">
            {([['classificacao', 'Classificação', ListOrdered], ['comparativo', 'Comparar vendedores', BarChart3]] as const).map(([key, label, Icon]) => (
              <button
                ...
                onClick={() => setView(key)}
                className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-medium transition-all ${view === key ? 'bg-emerald-600 text-white shadow-sm' : 'text-muted-foreground hover:bg-gray-50'}`}
              >
                <Icon size={15} /> {label}
              </button>
            ))}
        ...
        <section className="overflow-hidden rounded-2xl border border-border-subtle bg-white shadow-sm" aria-labelledby="ranking-table-title">
          ...
          {data.loading ? <div className="flex justify-center py-12"><div className="h-6 w-6 animate-spin rounded-full border-4 border-emerald-200 border-t-emerald-600" /></div> : ranking.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-center"><Trophy className="mb-3 text-text-disabled" size={48} /><p className="font-medium text-muted-foreground">Ainda não há dados suficientes para montar o ranking.</p></div> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="border-b border-border-subtle bg-gray-50"><tr>{RANKING_COLUMNS.map(({ label, help }) => <th key={label} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground"><span className="inline-flex items-center gap-1">{label}<HelpTooltip text={help} /></span></th>)}</tr></thead><tbody className="divide-y divide-gray-50">{ranking.map((seller, index) => <RankingRow key={seller.id} seller={seller} index={index} />)}</tbody></table></div>}
        </section>
        </>
        )}
    </PageCanvas>
...
function RankingRow({ seller, index }: { seller: RankedVendedor; index: number }) {
  ...
  const rowTone = index === 0 ? 'border-l-4 border-yellow-400 bg-yellow-50' : index === 1 ? 'border-l-4 border-border-strong bg-gray-50' : index === 2 ? 'border-l-4 border-amber-500 bg-orange-50' : ''
  const positionTone = index === 0 ? 'bg-yellow-400 text-white' : index === 1 ? 'bg-gray-400 text-white' : index === 2 ? 'bg-amber-600 text-white' : 'bg-gray-100 text-muted-foreground'
  const barTone = attainment === null ? 'bg-gray-300' : attainment >= 80 ? 'bg-emerald-500' : attainment >= 50 ? 'bg-amber-500' : 'bg-red-500'
  const textTone = attainment === null ? 'text-muted-foreground' : attainment >= 80 ? 'text-emerald-700' : attainment >= 50 ? 'text-amber-700' : 'text-red-700'
  return <tr className={rowTone}><td className="px-4 py-3"><span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${positionTone}`}>{index + 1}</span></td><td className="px-4 py-3 font-semibold text-foreground">{seller.nome}</td><td className="px-4 py-3 text-foreground">{seller.vendas}</td><td className="px-4 py-3 text-foreground">{seller.meta || '—'}</td><td className="px-4 py-3"><div className="flex min-w-[88px] items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-100"><div className={`h-full rounded-full ${barTone}`} style={{ width: `${Math.min(attainment ?? 0, 100)}%` }} /></div><span className={`text-xs font-medium ${textTone}`}>{attainment === null ? '—' : `${attainment}%`}</span></div></td><td className="px-4 py-3 text-foreground">{seller.agendamentos}</td><td className="px-4 py-3 text-foreground">{seller.conversao}%</td><td className="px-4 py-3 text-foreground">{seller.rotina === null ? '—' : `${seller.rotina}%`}</td><td className="px-4 py-3 font-bold text-emerald-700">{seller.pontuacao ?? '—'}</td></tr>
}
