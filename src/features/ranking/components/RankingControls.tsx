    ...
    <div role="group" aria-label="Período do ranking" className="flex flex-wrap items-center gap-1 rounded-xl bg-muted p-1">
      {RANKING_PERIODOS.map(periodo => (
        <button
          ...
          className={`min-h-11 rounded-lg px-4 text-sm font-semibold transition-all ${
            value === periodo
...
export function RankingUnitSelect({ id, value, unidades, onChange }: { id: string; value: string; unidades: string[]; onChange: (unidade: string) => void }) {
  ...
  return (
    <label className="flex items-center gap-2 text-sm font-semibold text-muted-foreground" htmlFor={id}>
      ...
      <select
        ...
        className="min-h-11 rounded-xl border border-border bg-white px-3 text-sm font-semibold text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary/40"
      >
...
export function RankingErrorNotice({ message, onRetry, isRetrying }: { message: string; onRetry: () => void; isRetrying: boolean }) {
  return (
    <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-status-error/30 bg-status-error-surface px-4 py-2 text-sm font-bold text-status-error-text">
      <span>{message}</span>
      <button
        type="button"
        onClick={onRetry}
        disabled={isRetrying}
        className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-status-error/40 bg-white px-4 text-sm font-bold text-status-error-text transition-colors hover:bg-status-error-surface disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-status-error/40"
      >
