    ...
    >
      <form id="digital-product-form" onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2"><Users size={16} className="text-status-success-text" /><Typography variant="caption" className="font-medium text-muted-foreground">Públicos onde aparece</Typography></div>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            {PRODUCT_AUDIENCES.map((audience) => {
              const selected = form.target_roles.includes(audience.key)
              return <button key={audience.key} type="button" disabled={!canManage} onClick={() => onToggleAudience(audience.key)} className={cn('min-h-20 rounded-xl border p-3 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-60', selected ? 'border-status-success/40 bg-status-success-surface text-foreground' : 'border-border bg-surface-alt text-muted-foreground hover:border-status-success/30')}><span className="block text-sm font-semibold">{audience.label}</span><span className="mt-1 block text-xs text-muted-foreground">{audience.description}</span></button>
            })}
          </div>
        </div>
