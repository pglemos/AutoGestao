...
function ProfileAvatar({ url, name }: { url: string; name: string }) {
  ...
  if (erro) return <div className="flex h-32 w-32 items-center justify-center rounded-2xl bg-brand-primary text-3xl font-bold text-white">{initials}</div>
    ...
    <MxModulePage id="internal-profile">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
        ...
        <div className="space-y-5 lg:col-span-8">
          ...
          <MxSectionCard><MxSectionHeader title="Segurança" description="A senha nunca é exibida ou armazenada nesta tela." actions={<Button variant="outline" onClick={() => state.setPasswordOpen(true)} disabled={!state.canEdit}><Key size={18} />Alterar senha</Button>} /><div className="flex items-center gap-3 p-5 text-sm text-muted-foreground"><ShieldCheck className="text-status-success-text" size={20} />Verificação de acesso ativa.</div></MxSectionCard>
        </div>
      </div>
      {state.passwordOpen ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface-overlay/50 p-4" role="dialog" aria-modal="true" aria-labelledby="internal-password-title" onMouseDown={event => { if (event.currentTarget === event.target) state.setPasswordOpen(false) }}><section className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl"><h2 id="internal-password-title" className="text-lg font-bold text-foreground">Alterar senha</h2><div className="mt-4 space-y-4"><MxField label="Nova senha"><Input type="password" value={state.newPassword} onChange={event => state.setNewPassword(event.target.value)} autoComplete="new-password" /></MxField><MxField label="Confirmar senha"><Input type="password" value={state.confirmPassword} onChange={event => state.setConfirmPassword(event.target.value)} autoComplete="new-password" /></MxField></div><div className="mt-5 flex justify-end gap-2"><Button variant="secondary" onClick={() => state.setPasswordOpen(false)} disabled={state.changingPassword}>Cancelar</Button><Button onClick={() => void state.updatePassword()} disabled={state.changingPassword}>{state.changingPassword ? <RefreshCw size={18} className="animate-spin motion-reduce:animate-none" /> : <ShieldCheck size={18} />}Atualizar senha</Button></div></section></div> : null}
    </MxModulePage>
  )
}
