  ...
  return (
    <MxModulePage id="internal-profile" bottomClearance="navigation">
      ...
      <Modal
        ...
        onClose={() => state.setPasswordOpen(false)}
        title="Alterar senha"
        size="sm"
        footer={
          <>
            <Button variant="outline" onClick={() => state.setPasswordOpen(false)} disabled={state.changingPassword}>Cancelar</Button>
            <Button onClick={() => void state.updatePassword()} disabled={state.changingPassword}>{state.changingPassword ? <RefreshCw size={18} className="animate-spin motion-reduce:animate-none" /> : <ShieldCheck size={18} />}Atualizar senha</Button>
          </>
        }
      >
        <div className="space-y-4">
          <MxField label="Nova senha"><Input type="password" value={state.newPassword} onChange={event => state.setNewPassword(event.target.value)} autoComplete="new-password" /></MxField>
          <MxField label="Confirmar senha"><Input type="password" value={state.confirmPassword} onChange={event => state.setConfirmPassword(event.target.value)} autoComplete="new-password" /></MxField>
        </div>
      </Modal>
    </MxModulePage>
  )
