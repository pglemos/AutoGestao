...
export function StrategicPlanningView({
  ...
  return (
    <div className="-mx-mx-sm min-h-full bg-gray-50 px-mx-sm py-mx-md md:-mx-mx-lg md:px-mx-lg">
      <div className="space-y-mx-md">
        ...
        <div className="inline-flex rounded-2xl border border-gray-100 bg-white p-0.5 shadow-sm">
          <button type="button" onClick={() => setTab('resumo')} className={cn('flex h-mx-10 items-center gap-mx-xs rounded-xl px-mx-md text-sm', tab === 'resumo' ? 'bg-status-success-surface font-bold text-emerald-600' : 'text-gray-500')}><BarChart3 size={16} /> Resumo</button>
          <button type="button" onClick={() => setTab('visao-geral')} className={cn('flex h-mx-10 items-center gap-mx-xs rounded-xl px-mx-md text-sm', tab === 'visao-geral' ? 'bg-status-success-surface font-bold text-emerald-600' : 'text-gray-500')}><List size={16} /> Visão Geral</button>
        </div>
          ...
          <>
            <Card className="border bg-white p-mx-sm">
              <div className="flex flex-col gap-mx-sm lg:flex-row lg:items-center">
                <label className="flex min-w-0 flex-1 items-center gap-mx-xs rounded-xl border border-gray-100 bg-white px-mx-sm text-sm"><Table2 size={15} className="text-gray-500" /><select aria-label="Indicador estratégico" value={selected.code} onChange={event => setSelectedCode(event.target.value)} className="h-mx-10 min-w-0 flex-1 bg-transparent outline-none"><option value={selected.code}>{selected.label}</option>{filtered.filter(indicator => indicator.code !== selected.code).map(indicator => <option key={indicator.code} value={indicator.code}>{indicator.label}</option>)}</select></label>
                <label className="flex items-center gap-mx-xs rounded-xl border border-gray-100 bg-white px-mx-sm text-sm"><Filter size={15} className="text-gray-500" /><select aria-label="Área estratégica" value={area} onChange={event => { setArea(event.target.value); setSelectedCode('') }} className="h-mx-10 bg-transparent outline-none"><option value="all">Todas as áreas</option>{areas.map(value => <option key={value} value={value}>{departmentLabel(value)}</option>)}</select></label>
                <div className="flex items-center gap-mx-xs text-mx-tiny text-gray-500"><span className="rounded-mx-full bg-gray-50 px-mx-sm py-mx-xs">Dados do período atual</span><button type="button" onClick={exportCsv} className="inline-flex items-center gap-mx-xs px-mx-xs font-bold text-gray-500"><Download size={15} /> Exportar</button><Button type="button" size="sm" onClick={() => navigate(`/dono/plano-acao?indicator=${encodeURIComponent(selected.code)}`)}><PlusCircle size={15} /> Criar Plano de Ação</Button></div>
              </div>
            </Card>
            ...
            <Card className="overflow-hidden border border-brand-primary/30 bg-white p-0">
              <div className="grid grid-cols-1 divide-y border-t-4 border-brand-primary md:grid-cols-[minmax(240px,1.2fr)_repeat(4,minmax(130px,1fr))] md:divide-x md:divide-y-0">
                <div className="p-mx-md"><Typography variant="h3" className="text-base font-bold">{selected.label}</Typography><div className="mt-mx-xs flex flex-wrap gap-mx-xs text-mx-tiny"><span className="rounded-mx-xs bg-emerald-600/10 px-mx-xs py-0.5 text-emerald-600">{departmentLabel(selected.department)}</span><span className="text-gray-500">{selected.code}</span><span className={cn('rounded-mx-xs px-mx-xs py-0.5', planningStatusTone(selected.status))}>{statusLabel(selected.status)}</span></div></div>
...
function PlanningRow({ label, value, unit, month }: { label: string; value: number | null | undefined; unit: CentralMxIndicatorValue['unit']; month: number }) {
  return <tr className="border-t border-gray-100"><td className="px-mx-sm py-mx-sm font-bold text-gray-500">{label}</td>{MONTHS.map((name, index) => <td key={name} className={cn('px-mx-sm py-mx-sm text-center tabular-nums', index === month && 'bg-emerald-600/5 font-bold text-emerald-600')}>{index === month ? formatPlanningValue(value, unit) : '—'}</td>)}<td className="px-mx-sm py-mx-sm text-center font-bold tabular-nums">{formatPlanningValue(value, unit)}</td></tr>
}
