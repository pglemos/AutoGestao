...
import { ArrowDownUp, BarChart3, Filter, List, PlusCircle, Table2, Download } from 'lucide-react'
  ...
  return (
    <div className="-mx-mx-sm min-h-full bg-surface-alt px-mx-sm py-mx-md md:-mx-mx-lg md:px-mx-lg">
      <div className="space-y-mx-md">
        ...
        <div className="inline-flex rounded-mx-xl border border-border-subtle bg-white p-0.5 shadow-mx-sm">
          <button type="button" onClick={() => setTab('resumo')} className={cn('flex h-mx-10 items-center gap-mx-xs rounded-mx-lg px-mx-md text-sm', tab === 'resumo' ? 'bg-status-success-surface font-bold text-brand-primary' : 'text-text-secondary')}><BarChart3 size={16} /> Resumo</button>
          <button type="button" onClick={() => setTab('visao-geral')} className={cn('flex h-mx-10 items-center gap-mx-xs rounded-mx-lg px-mx-md text-sm', tab === 'visao-geral' ? 'bg-status-success-surface font-bold text-brand-primary' : 'text-text-secondary')}><List size={16} /> Visão Geral</button>
        </div>
          ...
          <>
            <Card className="rounded-mx-xl border border-border-subtle bg-white p-mx-sm shadow-mx-sm">
              <div className="flex flex-col gap-mx-sm lg:flex-row lg:items-center">
                <label className="flex min-w-0 flex-1 items-center gap-mx-xs rounded-mx-lg border border-border-subtle bg-white px-mx-sm text-sm"><Table2 size={15} className="text-text-tertiary" /><select aria-label="Indicador estratégico" value={selected.code} onChange={event => setSelectedCode(event.target.value)} className="h-mx-10 min-w-0 flex-1 bg-transparent outline-none"><option value={selected.code}>{selected.label}</option>{filtered.filter(indicator => indicator.code !== selected.code).map(indicator => <option key={indicator.code} value={indicator.code}>{indicator.label}</option>)}</select></label>
                <label className="flex items-center gap-mx-xs rounded-mx-lg border border-border-subtle bg-white px-mx-sm text-sm"><Filter size={15} className="text-text-tertiary" /><select aria-label="Área estratégica" value={area} onChange={event => { setArea(event.target.value); setSelectedCode('') }} className="h-mx-10 bg-transparent outline-none"><option value="all">Todas as áreas</option>{areas.map(value => <option key={value} value={value}>{departmentLabel(value)}</option>)}</select></label>
                <div className="flex items-center gap-mx-xs text-mx-tiny text-text-secondary"><span className="rounded-mx-full bg-surface-alt px-mx-sm py-mx-xs">Dados do período atual</span><button type="button" onClick={exportCsv} className="inline-flex items-center gap-mx-xs px-mx-xs font-bold text-text-secondary"><Download size={15} /> Exportar</button><Button type="button" size="sm" onClick={() => navigate(`/dono/plano-acao?indicator=${encodeURIComponent(selected.code)}`)}><PlusCircle size={15} /> Criar Plano de Ação</Button></div>
              </div>
            </Card>

            <Card className="overflow-hidden rounded-mx-xl border border-brand-primary/30 bg-white p-0 shadow-mx-sm">
              <div className="grid grid-cols-1 divide-y border-t-4 border-brand-primary md:grid-cols-[minmax(240px,1.2fr)_repeat(4,minmax(130px,1fr))] md:divide-x md:divide-y-0">
                <div className="p-mx-md"><Typography variant="h3" className="text-base font-bold">{selected.label}</Typography><div className="mt-mx-xs flex flex-wrap gap-mx-xs text-mx-tiny"><span className="rounded-mx-xs bg-brand-primary/10 px-mx-xs py-0.5 text-brand-primary">{departmentLabel(selected.department)}</span><span className="text-text-tertiary">{selected.code}</span><span className={cn('rounded-mx-xs px-mx-xs py-0.5', planningStatusTone(selected.status))}>{statusLabel(selected.status)}</span></div></div>
            ...
            <Card className="overflow-hidden rounded-mx-xl border border-border-subtle bg-white p-0 shadow-mx-sm">
              ...
              <div className="overflow-x-auto"><table className="min-w-[1050px] w-full text-sm"><thead><tr className="bg-surface-alt text-left text-mx-tiny font-bold text-text-secondary"><th className="px-mx-sm py-mx-sm">Comparativo</th>{MONTHS.map((label, index) => <th key={label} className={cn('px-mx-sm py-mx-sm text-center', index === month && 'bg-brand-primary/5 text-brand-primary')}>{label}</th>)}<th className="px-mx-sm py-mx-sm text-center">Período atual</th></tr></thead><tbody><PlanningRow label="Meta" value={selected.meta == null ? null : selected.meta} unit={selected.unit} month={month} /><PlanningRow label="Resultado Atual" value={selected.realizado == null ? null : selected.realizado} unit={selected.unit} month={month} /><PlanningRow label="Score" value={selected.score} unit="score" month={month} /><PlanningRow label="Ano Anterior" value={selected.anoAnterior} unit={selected.unit} month={month} /></tbody></table></div>
            </Card>

            <div className="grid gap-mx-md lg:grid-cols-2"><Card className="rounded-mx-xl border border-border-subtle bg-white p-mx-md"><Typography variant="h3" className="text-base font-bold">Leitura do indicador</Typography><Typography variant="p" className="mt-mx-sm text-sm text-text-secondary">{selected.realizado == null ? 'O resultado ainda não foi registrado para este indicador.' : `O resultado atual é ${formatPlanningValue(selected.realizado, selected.unit)}${selected.meta == null ? '.' : ` diante de uma meta de ${formatPlanningValue(selected.meta, selected.unit)}.`}`}</Typography></Card><Card className="rounded-mx-xl border border-status-warning/30 bg-status-warning-surface p-mx-md"><Typography variant="h3" className="text-base font-bold">Direcionamento MX</Typography><Typography variant="p" className="mt-mx-sm text-sm text-text-secondary">Use o Plano de Ação para registrar a decisão, responsável, prazo e evidência deste indicador.</Typography><Button type="button" variant="outline" className="mt-mx-md" onClick={() => navigate(`/dono/plano-acao?indicator=${encodeURIComponent(selected.code)}`)}>Criar Plano de Ação <ArrowDownUp size={15} /></Button></Card></div>
          </>
        )}

        {tab === 'visao-geral' && <Card className="overflow-hidden rounded-mx-xl border border-border-subtle bg-white p-0 shadow-mx-sm"><div className="overflow-x-auto"><table className="min-w-[900px] w-full text-sm"><thead className="bg-surface-alt text-left text-mx-tiny font-bold text-text-secondary"><tr><th className="px-mx-md py-mx-sm">Indicador</th><th className="px-mx-md py-mx-sm">Departamento</th><th className="px-mx-md py-mx-sm text-right">Meta</th><th className="px-mx-md py-mx-sm text-right">Realizado</th><th className="px-mx-md py-mx-sm text-right">Score</th><th className="px-mx-md py-mx-sm">Status</th></tr></thead><tbody className="divide-y divide-border-subtle">{filtered.map(indicator => <tr key={indicator.code} className="cursor-pointer hover:bg-surface-alt" onClick={() => { setSelectedCode(indicator.code); setTab('resumo') }}><td className="px-mx-md py-mx-sm font-bold">{indicator.label}</td><td className="px-mx-md py-mx-sm text-text-secondary">{departmentLabel(indicator.department)}</td><td className="px-mx-md py-mx-sm text-right tabular-nums">{formatPlanningValue(indicator.meta, indicator.unit)}</td><td className="px-mx-md py-mx-sm text-right tabular-nums">{formatPlanningValue(indicator.realizado, indicator.unit)}</td><td className="px-mx-md py-mx-sm text-right tabular-nums">{indicator.score ?? '--'}</td><td className="px-mx-md py-mx-sm"><span className={cn('rounded-mx-xs px-mx-xs py-0.5 text-mx-tiny', planningStatusTone(indicator.status))}>{statusLabel(indicator.status)}</span></td></tr>)}</tbody></table></div></Card>}
      </div>
    </div>
  )
}
...
function PlanningRow({ label, value, unit, month }: { label: string; value: number | null | undefined; unit: CentralMxIndicatorValue['unit']; month: number }) {
  return <tr className="border-t border-border-subtle"><td className="px-mx-sm py-mx-sm font-bold text-text-secondary">{label}</td>{MONTHS.map((name, index) => <td key={name} className={cn('px-mx-sm py-mx-sm text-center tabular-nums', index === month && 'bg-brand-primary/5 font-bold text-brand-primary')}>{index === month ? formatPlanningValue(value, unit) : '—'}</td>)}<td className="px-mx-sm py-mx-sm text-center font-bold tabular-nums">{formatPlanningValue(value, unit)}</td></tr>
}
