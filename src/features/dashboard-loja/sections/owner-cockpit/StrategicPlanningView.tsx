          ...
          <>
            ...
            <Card className="overflow-hidden border border-brand-primary/30 bg-white p-0">
              <div className="grid grid-cols-1 divide-y border-t-4 border-brand-primary md:grid-cols-[minmax(240px,1.2fr)_repeat(4,minmax(130px,1fr))] md:divide-x md:divide-y-0">
                <div className="p-mx-md"><Typography variant="h3" className="text-base font-bold">{selected.label}</Typography><div className="mt-mx-xs flex flex-wrap gap-mx-xs text-mx-tiny"><span className="rounded-mx-xs bg-emerald-600/10 px-mx-xs py-0.5 text-emerald-600">{departmentLabel(selected.department)}</span><span className="text-muted-foreground">{selected.code}</span><span className={cn('rounded-mx-xs px-mx-xs py-0.5', planningStatusTone(selected.status))}>{statusLabel(selected.status)}</span></div></div>
