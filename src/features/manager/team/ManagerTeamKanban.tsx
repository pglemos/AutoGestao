    ...
    <div className="space-y-3 md:hidden">
      <div className="grid grid-cols-3 gap-2">
        {STATUS_ORDER.map(status => {
          ...
          return <button key={status} type="button" disabled={groups[status].length === 0} onClick={() => setMobileStatus(status)} className={`rounded-xl border py-2.5 transition-colors ${active ? config.active : groups[status].length === 0 ? 'border-gray-100 bg-gray-50 text-text-disabled' : `${config.header} bg-opacity-60`}`}>
...
function KanbanMobileList(props: ColumnProps) {
  ...
  return <>{props.cards.length ? <div className="space-y-3">{props.cards.map(card => <ManagerSellerCard key={card.row.user_id} card={card} view={props.view} status={props.status} storeName={props.storeName} onOpenProfile={props.onOpenProfile} onAction={props.onAction}/>)}</div> : <div className="rounded-2xl border border-gray-100 bg-white py-12 text-center text-sm text-muted-foreground">Nenhum vendedor nesta coluna</div>}<div className={`mt-3 rounded-2xl border p-3 ${config.footer}`}><p className="text-xs font-semibold">{guidance.title}</p><p className="mt-0.5 text-caption leading-snug opacity-75">{guidance.detail}</p></div></>
}
...
function KanbanMetric({ label, value, status, highlight }: { label: string; value: number | null; status: ManagerTeamStatus; highlight: boolean }) {
  ...
  return <div className="rounded-xl border border-gray-100 bg-gray-50 p-2.5 text-center"><p className={`text-xl font-bold ${valueTone}`}>{value === null ? '—' : `${Math.round(value)}%`}</p><p className="text-xs text-muted-foreground">{label}</p></div>
}
