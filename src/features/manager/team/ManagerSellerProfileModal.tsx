...
function Metric({ icon: Icon, label, value, compact = false, tone = 'default' }: { icon: typeof TrendingUp; label: string; value: string | number; compact?: boolean; tone?: 'default' | 'success' | 'info' | 'attention' | 'purple' }) { const colors = { default: 'text-muted-foreground text-foreground', success: 'text-status-success text-status-success-text', info: 'text-status-info text-status-info-text', attention: 'text-status-warning text-status-warning-text', purple: 'text-violet-500 text-violet-600' }[tone].split(' '); return <div className={`rounded-xl bg-gray-50 ${compact ? 'p-3' : 'p-3.5'}`}><Icon size={16} className={colors[0]}/><p className={`mt-1 font-bold ${compact ? 'text-lg' : 'text-2xl'} ${colors[1]}`}>{value}</p><p className="text-xs text-muted-foreground">{label}</p></div> }
