...
function OverviewTab({ seller, card, result, consistency, sellerTarget, status }: { seller: RankingEntry; card: ManagerTeamCard; result: number | null; consistency: number | null; sellerTarget: number | null; status: ReturnType<typeof statusCopy> }) {
  return <>
    ...
    <section className="rounded-xl bg-gray-50 p-4"><h3 className="text-sm font-bold text-foreground">Diagnóstico atual</h3><p className="mt-2 text-sm text-muted-foreground">{card.reason}</p><p className="mt-3 flex items-center gap-2 text-xs text-status-success-text"><CheckCircle2 size={14} /> Ponto positivo: <strong>—</strong></p></section>
...
function RoutineTab({ card, onOpenRoutine }: { card: ManagerTeamCard; onOpenRoutine: () => void }) {
  return <div className="space-y-5"><section className="grid min-h-44 place-items-center rounded-xl bg-gray-50 px-5 py-12 text-center"><div><Activity className="mx-auto mb-3 h-10 w-10 text-text-disabled"/><p className="text-sm text-muted-foreground">{card.routine === null ? 'Não há dados de rotina para o período selecionado.' : `Execução verificada: ${Math.round(card.routine)}%.`}</p></div></section><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-brand-primary px-3 text-xs font-medium text-white hover:bg-brand-primary-hover" onClick={onOpenRoutine}>Abrir Rotina da Equipe <span aria-hidden="true">→</span></button></div>
}

function FeedbacksTab({ onOpenFeedback }: { onOpenFeedback: () => void }) {
  return <div className="space-y-5"><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-brand-primary px-3 text-xs font-medium text-white hover:bg-brand-primary-hover" onClick={onOpenFeedback}><Plus size={14}/>Novo Feedback</button><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl border border-border px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onOpenFeedback}><Eye size={14}/>Ver histórico completo</button></div><section className="rounded-xl border border-border-subtle bg-white p-4"><h3 className="text-sm font-semibold text-foreground">PDI ativo</h3><EmptyPanel icon={FileText} text="Nenhum PDI ativo." compact /></section><section className="rounded-xl border border-border-subtle bg-white p-4"><h3 className="text-sm font-semibold text-foreground">Histórico de feedbacks</h3><EmptyPanel icon={MessageSquare} text="Nenhum feedback registrado para este vendedor." action="Registrar feedback" onAction={onOpenFeedback} compact /></section></div>
}

function TrainingTab({ onOpenTraining }: { onOpenTraining: () => void }) {
  return <div className="space-y-5"><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-brand-primary px-3 text-xs font-medium text-white hover:bg-brand-primary-hover" onClick={onOpenTraining}><Plus size={14}/>Recomendar treinamento</button><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl border border-border px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onOpenTraining}><Eye size={14}/>Ver acompanhamento completo</button></div><section className="rounded-xl border border-border-subtle bg-white p-4"><h3 className="mb-3 text-sm font-semibold text-foreground">Acompanhamento de treinamentos</h3><div className="grid grid-cols-2 gap-3 md:grid-cols-4"><Metric icon={BookOpen} label="Trilha atual" value="—" compact /><Metric icon={CheckCircle2} label="Progresso geral" value="—" compact /><Metric icon={Award} label="Certificados" value="—" compact /><Metric icon={Clock} label="Último acesso" value="—" compact /></div></section><section className="rounded-xl bg-gray-50"><EmptyPanel icon={GraduationCap} text="Nenhum treinamento atribuído a este vendedor." action="Recomendar treinamento" onAction={onOpenTraining} /></section></div>
}
...
function Metric({ icon: Icon, label, value, compact = false, tone = 'default' }: { icon: typeof TrendingUp; label: string; value: string | number; compact?: boolean; tone?: 'default' | 'success' | 'info' | 'attention' | 'purple' }) { const colors = { default: 'text-muted-foreground text-foreground', success: 'text-status-success text-status-success-text', info: 'text-status-info text-status-info-text', attention: 'text-status-warning text-status-warning-text', purple: 'text-status-info-text text-status-info-text' }[tone].split(' '); return <div className={`rounded-xl bg-gray-50 ${compact ? 'p-3' : 'p-3.5'}`}><Icon size={16} className={colors[0]}/><p className={`mt-1 font-bold ${compact ? 'text-lg' : 'text-2xl'} ${colors[1]}`}>{value}</p><p className="text-xs text-muted-foreground">{label}</p></div> }
...
function statusCopy(status: ManagerTeamCard['overallStatus']) { return status === 'on_track' ? { label: 'Em dia', badge: 'bg-status-success-surface text-status-success-text', tone: 'success' as const } : status === 'attention' ? { label: 'Atenção', badge: 'bg-status-warning-surface text-status-warning-text', tone: 'attention' as const } : status === 'not_applicable' ? { label: 'Não aplicável', badge: 'bg-gray-100 text-muted-foreground', tone: 'muted' as const } : { label: 'Crítico', badge: 'bg-status-error-surface text-status-error-text', tone: 'critical' as const } }
