  ...
  return <>
    ...
    <details className="group overflow-hidden rounded-xl border border-gray-200">
      ...
      <div className="divide-y divide-gray-100 border-t border-gray-100 bg-gray-50 px-4 text-sm text-gray-600">
        ...
        <p className="pt-3 text-caption text-gray-400">Fórmula: Rotina × 0,70 + Disciplina × 0,30</p>
    ...
    <section className="rounded-xl border border-gray-200 p-4">
      ...
      <div className="mt-4 grid gap-5 md:grid-cols-2">
        ...
        <div className="space-y-2 text-sm"><StatusLine label="Status Geral" value={status.label} tooltip="Classificação matricial combinando Resultado e Consistência (Em dia, Atenção ou Crítico)." /><StatusLine label="Status por Resultado" value={statusFromResult(result)} /><StatusLine label="Status por Consistência" value={statusFromConsistency(consistency)} /><p className="pt-1 text-xs text-gray-600"><span className="text-gray-400">Motivo: </span>{card.reason}</p><StatusLine label="Índice Gerencial" value={card.managementIndex === null ? '—' : `${Math.round(card.managementIndex)} pontos`} helper="Índice de apoio à priorização — não determina o status." tooltip="Índice numérico de 0 a 100 para priorização gerencial da equipe (50% Resultado + 50% Consistência)." /></div>
      </div>
...
function HeroMetric({ icon: Icon, label, value, detail, tone, tooltip }: { icon: typeof TrendingUp; label: string; value: string; detail: string; tone: 'critical' | 'attention' | 'success' | 'muted'; tooltip?: string }) {
  ...
  return <div className={`rounded-xl border p-4 ${theme}`}><div className="flex items-center justify-between text-sm font-bold"><div className="flex items-center gap-2"><Icon size={15} />{label}</div>{tooltip && <HelpTooltip text={tooltip} />}</div><p className="mt-3 text-2xl font-bold">{value}</p><p className="mt-1 text-xs text-gray-500">{detail}</p></div>
}
...
function StatusLine({ label, value, helper, tooltip }: { label: string; value: string; helper?: string; tooltip?: string }) { return <div className="flex items-start justify-between gap-4"><span className="inline-flex items-center gap-1.5 text-gray-500">{label}{tooltip && <HelpTooltip text={tooltip} />}:</span><span className="text-right font-semibold text-gray-700">{value}{helper && <small className="mt-0.5 block text-caption font-normal text-gray-400">{helper}</small>}</span></div> }
