      ...
      <section role="dialog" aria-modal="true" aria-label={`Perfil de ${seller.user_name}`} className="z-[120] flex max-h-[92vh] w-[90vw] max-w-7xl flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
        <header className="flex shrink-0 flex-wrap items-start justify-between gap-2 border-b border-gray-100 px-6 py-4 sm:gap-4">
          ...
          <div className="order-first flex w-full shrink-0 translate-x-6 items-center justify-end gap-2 sm:order-none sm:translate-x-0 sm:w-auto">
            ...
            <button type="button" aria-label="Fechar perfil do vendedor" className="ml-1 hidden text-muted-foreground hover:text-muted-foreground sm:inline-flex" onClick={onClose}><X size={20}/></button>
          </div>
        </header>
...
function OverviewTab({ seller, card, result, consistency, sellerTarget, status }: { seller: RankingEntry; card: ManagerTeamCard; result: number | null; consistency: number | null; sellerTarget: number | null; status: ReturnType<typeof statusCopy> }) {
  return <>
    ...
    <details className="group overflow-hidden rounded-xl border border-gray-200">
      <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-semibold text-foreground [&::-webkit-details-marker]:hidden"><span className="flex items-center gap-2"><Activity size={15} className="text-emerald-600" /> Composição da Consistência <HelpTooltip text="Combina 70% de execução de Rotina e 30% de Disciplina do Fechamento diário." /></span><ChevronDown size={16} className="text-muted-foreground transition-transform group-open:rotate-180" /></summary>
      <div className="divide-y divide-gray-100 border-t border-gray-100 bg-gray-50 px-4 text-sm text-muted-foreground">
    ...
    <section className="rounded-xl bg-gray-50 p-4"><h3 className="text-sm font-bold text-foreground">Diagnóstico atual</h3><p className="mt-2 text-sm text-muted-foreground">{card.reason}</p><p className="mt-3 flex items-center gap-2 text-xs text-emerald-700"><CheckCircle2 size={14} /> Ponto positivo: <strong>—</strong></p></section>

    <section className="rounded-xl border border-gray-200 p-4"><h3 className="text-sm font-bold text-foreground">Informações gerenciais</h3><div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3"><Info label="Data da última venda" value="—" /><Info label="Dias sem vender" value="—" /><Info label="Último feedback" value="—" /><Info label="Próximo feedback agendado" value="—" /><Info label="PDI ativo" value="Nenhum PDI ativo" /><Info label="Próximo compromisso do PDI" value="—" /><Info label="Treinamentos pendentes" value="—" /><Info label="Último acesso à Universidade MX" value="—" /></div></section>
  </>
}
...
function PerformanceTab({ seller, card, period, onPeriodChange }: { seller: RankingEntry; card: ManagerTeamCard; period: PerformancePeriod; onPeriodChange: (period: PerformancePeriod) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border border-gray-100 bg-white p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="text-sm font-bold text-foreground">Vendas acumuladas × Meta acumulada</h3><div className="inline-flex rounded-xl bg-gray-50 p-1"><button type="button" aria-pressed={period === 'current'} onClick={() => onPeriodChange('current')} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${period === 'current' ? 'bg-emerald-600 text-white' : 'text-muted-foreground hover:text-foreground'}`}>Mês atual</button><button type="button" aria-pressed={period === 'quarter'} onClick={() => onPeriodChange('quarter')} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${period === 'quarter' ? 'bg-emerald-600 text-white' : 'text-muted-foreground hover:text-foreground'}`}>Últimos 3 meses</button></div></div>
      <div className="relative mt-4 h-60 overflow-hidden rounded-lg border border-gray-100 bg-[linear-gradient(#f3f4f6_1px,transparent_1px),linear-gradient(90deg,#f3f4f6_1px,transparent_1px)] bg-[size:32px_32px]" aria-label={`Série acumulada — ${period === 'current' ? 'Mês atual' : 'Últimos 3 meses'}`}><div className="absolute inset-x-5 bottom-12 border-t border-gray-300" /><div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-3 whitespace-nowrap text-xs"><span className="text-emerald-500">— Vendas acumuladas</span><span className="text-muted-foreground">— Meta acumulada</span></div><p className="absolute bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap text-caption text-muted-foreground">Série diária indisponível no contrato atual.</p></div>
    </section>
    <section className="rounded-xl border border-gray-100 bg-white p-4"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-bold text-foreground">Resultado por canal</h3><span className="text-xs text-muted-foreground">Leads registrados no MX</span></div><div className="mt-4 grid gap-3 md:grid-cols-3"><ChannelMetric label="Showroom" base="atendimentos" /><ChannelMetric label="Carteira" base="contatos/leads" /><ChannelMetric label="Internet" base="leads" /><ChannelMetric label="Atendimento anterior / Sem canal" base="Base" sales={seller.vnd_total} /></div><div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3 text-sm font-semibold text-foreground"><span>Total de vendas no período</span><span>{seller.vnd_total}</span></div></section>
  </div>
}

function RoutineTab({ card, onOpenRoutine }: { card: ManagerTeamCard; onOpenRoutine: () => void }) {
  return <div className="space-y-5"><section className="grid min-h-44 place-items-center rounded-xl bg-gray-50 px-5 py-12 text-center"><div><Activity className="mx-auto mb-3 h-10 w-10 text-text-disabled"/><p className="text-sm text-muted-foreground">{card.routine === null ? 'Não há dados de rotina para o período selecionado.' : `Execução verificada: ${Math.round(card.routine)}%.`}</p></div></section><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-emerald-600 px-3 text-xs font-medium text-white hover:bg-emerald-700" onClick={onOpenRoutine}>Abrir Rotina da Equipe <span aria-hidden="true">→</span></button></div>
}

function FeedbacksTab({ onOpenFeedback }: { onOpenFeedback: () => void }) {
  return <div className="space-y-5"><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-emerald-600 px-3 text-xs font-medium text-white hover:bg-emerald-700" onClick={onOpenFeedback}><Plus size={14}/>Novo Feedback</button><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl border border-gray-200 px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onOpenFeedback}><Eye size={14}/>Ver histórico completo</button></div><section className="rounded-xl border border-gray-100 bg-white p-4"><h3 className="text-sm font-semibold text-foreground">PDI ativo</h3><EmptyPanel icon={FileText} text="Nenhum PDI ativo." compact /></section><section className="rounded-xl border border-gray-100 bg-white p-4"><h3 className="text-sm font-semibold text-foreground">Histórico de feedbacks</h3><EmptyPanel icon={MessageSquare} text="Nenhum feedback registrado para este vendedor." action="Registrar feedback" onAction={onOpenFeedback} compact /></section></div>
}

function TrainingTab({ onOpenTraining }: { onOpenTraining: () => void }) {
  return <div className="space-y-5"><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-emerald-600 px-3 text-xs font-medium text-white hover:bg-emerald-700" onClick={onOpenTraining}><Plus size={14}/>Recomendar treinamento</button><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl border border-gray-200 px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onOpenTraining}><Eye size={14}/>Ver acompanhamento completo</button></div><section className="rounded-xl border border-gray-100 bg-white p-4"><h3 className="mb-3 text-sm font-semibold text-foreground">Acompanhamento de treinamentos</h3><div className="grid grid-cols-2 gap-3 md:grid-cols-4"><Metric icon={BookOpen} label="Trilha atual" value="—" compact /><Metric icon={CheckCircle2} label="Progresso geral" value="—" compact /><Metric icon={Award} label="Certificados" value="—" compact /><Metric icon={Clock} label="Último acesso" value="—" compact /></div></section><section className="rounded-xl bg-gray-50"><EmptyPanel icon={GraduationCap} text="Nenhum treinamento atribuído a este vendedor." action="Recomendar treinamento" onAction={onOpenTraining} /></section></div>
}
...
function HeroMetric({ icon: Icon, label, value, detail, tone, tooltip }: { icon: typeof TrendingUp; label: string; value: string; detail: string; tone: 'critical' | 'attention' | 'success' | 'muted'; tooltip?: string }) {
  const theme = { critical: 'border-red-100 bg-red-50 text-red-600', attention: 'border-amber-100 bg-amber-50 text-amber-600', success: 'border-emerald-100 bg-emerald-50 text-emerald-600', muted: 'border-gray-100 bg-gray-50 text-muted-foreground' }[tone]
...
function Metric({ icon: Icon, label, value, compact = false, tone = 'default' }: { icon: typeof TrendingUp; label: string; value: string | number; compact?: boolean; tone?: 'default' | 'success' | 'info' | 'attention' | 'purple' }) { const colors = { default: 'text-muted-foreground text-foreground', success: 'text-emerald-500 text-emerald-600', info: 'text-blue-500 text-blue-600', attention: 'text-amber-500 text-amber-600', purple: 'text-violet-500 text-violet-600' }[tone].split(' '); return <div className={`rounded-xl bg-gray-50 ${compact ? 'p-3' : 'p-3.5'}`}><Icon size={16} className={colors[0]}/><p className={`mt-1 font-bold ${compact ? 'text-lg' : 'text-2xl'} ${colors[1]}`}>{value}</p><p className="text-xs text-muted-foreground">{label}</p></div> }
...
function EmptyPanel({ icon: Icon, text, action, onAction, compact = false }: { icon: typeof FileText; text: string; action?: string; onAction?: () => void; compact?: boolean }) { return <div className={`flex flex-col items-center justify-center text-center ${compact ? 'py-8' : 'px-5 py-12'}`}><Icon className="mb-2 h-8 w-8 text-text-disabled"/><p className="text-sm text-muted-foreground">{text}</p>{action && onAction ? <button type="button" className="mt-3 inline-flex h-9 items-center gap-1 rounded-xl border border-gray-200 bg-white px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onAction}><Plus size={14}/>{action}</button> : null}</div> }
...
function statusCopy(status: ManagerTeamCard['overallStatus']) { return status === 'on_track' ? { label: 'Em dia', badge: 'bg-emerald-100 text-emerald-700', tone: 'success' as const } : status === 'attention' ? { label: 'Atenção', badge: 'bg-amber-100 text-amber-700', tone: 'attention' as const } : status === 'not_applicable' ? { label: 'Não aplicável', badge: 'bg-gray-100 text-muted-foreground', tone: 'muted' as const } : { label: 'Crítico', badge: 'bg-red-100 text-red-600', tone: 'critical' as const } }
