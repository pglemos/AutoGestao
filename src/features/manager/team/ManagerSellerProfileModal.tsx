...
function OverviewTab({ seller, card, result, consistency, sellerTarget, status }: { seller: RankingEntry; card: ManagerTeamCard; result: number | null; consistency: number | null; sellerTarget: number | null; status: ReturnType<typeof statusCopy> }) {
  return <>
    ...
    <details className="group overflow-hidden rounded-xl border border-border">
      ...
      <div className="divide-y divide-border-subtle border-t border-border-subtle bg-gray-50 px-4 text-sm text-muted-foreground">
...
function PerformanceTab({ seller, card, period, onPeriodChange }: { seller: RankingEntry; card: ManagerTeamCard; period: PerformancePeriod; onPeriodChange: (period: PerformancePeriod) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border border-border-subtle bg-white p-4">
      ...
      <div className="relative mt-4 h-60 overflow-hidden rounded-lg border border-border-subtle bg-[linear-gradient(#f3f4f6_1px,transparent_1px),linear-gradient(90deg,#f3f4f6_1px,transparent_1px)] bg-[size:32px_32px]" aria-label={`Série acumulada — ${period === 'current' ? 'Mês atual' : 'Últimos 3 meses'}`}><div className="absolute inset-x-5 bottom-12 border-t border-border-strong" /><div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-3 whitespace-nowrap text-xs"><span className="text-emerald-500">— Vendas acumuladas</span><span className="text-muted-foreground">— Meta acumulada</span></div><p className="absolute bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap text-caption text-muted-foreground">Série diária indisponível no contrato atual.</p></div>
    </section>
...
function FeedbacksTab({ onOpenFeedback }: { onOpenFeedback: () => void }) {
  return <div className="space-y-5"><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-emerald-600 px-3 text-xs font-medium text-white hover:bg-emerald-700" onClick={onOpenFeedback}><Plus size={14}/>Novo Feedback</button><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl border border-border px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onOpenFeedback}><Eye size={14}/>Ver histórico completo</button></div><section className="rounded-xl border border-border-subtle bg-white p-4"><h3 className="text-sm font-semibold text-foreground">PDI ativo</h3><EmptyPanel icon={FileText} text="Nenhum PDI ativo." compact /></section><section className="rounded-xl border border-border-subtle bg-white p-4"><h3 className="text-sm font-semibold text-foreground">Histórico de feedbacks</h3><EmptyPanel icon={MessageSquare} text="Nenhum feedback registrado para este vendedor." action="Registrar feedback" onAction={onOpenFeedback} compact /></section></div>
}

function TrainingTab({ onOpenTraining }: { onOpenTraining: () => void }) {
  return <div className="space-y-5"><div className="flex flex-wrap gap-2"><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl bg-emerald-600 px-3 text-xs font-medium text-white hover:bg-emerald-700" onClick={onOpenTraining}><Plus size={14}/>Recomendar treinamento</button><button type="button" className="inline-flex h-9 items-center gap-1 rounded-xl border border-border px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onOpenTraining}><Eye size={14}/>Ver acompanhamento completo</button></div><section className="rounded-xl border border-border-subtle bg-white p-4"><h3 className="mb-3 text-sm font-semibold text-foreground">Acompanhamento de treinamentos</h3><div className="grid grid-cols-2 gap-3 md:grid-cols-4"><Metric icon={BookOpen} label="Trilha atual" value="—" compact /><Metric icon={CheckCircle2} label="Progresso geral" value="—" compact /><Metric icon={Award} label="Certificados" value="—" compact /><Metric icon={Clock} label="Último acesso" value="—" compact /></div></section><section className="rounded-xl bg-gray-50"><EmptyPanel icon={GraduationCap} text="Nenhum treinamento atribuído a este vendedor." action="Recomendar treinamento" onAction={onOpenTraining} /></section></div>
}
...
function HeroMetric({ icon: Icon, label, value, detail, tone, tooltip }: { icon: typeof TrendingUp; label: string; value: string; detail: string; tone: 'critical' | 'attention' | 'success' | 'muted'; tooltip?: string }) {
  const theme = { critical: 'border-red-100 bg-red-50 text-red-600', attention: 'border-amber-100 bg-amber-50 text-amber-600', success: 'border-emerald-100 bg-emerald-50 text-emerald-600', muted: 'border-border-subtle bg-gray-50 text-muted-foreground' }[tone]
...
function EmptyPanel({ icon: Icon, text, action, onAction, compact = false }: { icon: typeof FileText; text: string; action?: string; onAction?: () => void; compact?: boolean }) { return <div className={`flex flex-col items-center justify-center text-center ${compact ? 'py-8' : 'px-5 py-12'}`}><Icon className="mb-2 h-8 w-8 text-text-disabled"/><p className="text-sm text-muted-foreground">{text}</p>{action && onAction ? <button type="button" className="mt-3 inline-flex h-9 items-center gap-1 rounded-xl border border-border bg-white px-3 text-xs font-medium text-foreground hover:bg-gray-50" onClick={onAction}><Plus size={14}/>{action}</button> : null}</div> }
