...
export default function ManagerTeamRoutineCanonical() {
  ...
  return <div className="min-h-full bg-gray-50"><div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-6 pb-24">
    ...
    <header className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><h1 className="text-xl font-bold text-foreground">Rotina da Equipe</h1><p className="mt-0.5 text-sm text-muted-foreground">Acompanhe a execução oficial das atividades comerciais da equipe.</p></div><div className="flex flex-wrap items-end gap-2"><label className="text-xs text-muted-foreground" htmlFor="manager-routine-date">Data<input id="manager-routine-date" type="date" value={date} onChange={event => setDate(event.target.value)} className="mt-1 block h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500"/></label><label className="text-xs text-muted-foreground">Buscar<div className="relative mt-1"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground"/><input aria-label="Vendedor..." value={search} onChange={event => setSearch(event.target.value)} placeholder="Vendedor..." className="h-10 w-40 rounded-xl border border-gray-200 pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500"/></div></label><button type="button" className="flex h-10 items-center gap-1 rounded-xl bg-emerald-600 px-4 text-sm font-medium text-white hover:bg-emerald-700" onClick={refresh}><RefreshCw size={15}/>Atualizar</button></div></div></header>
