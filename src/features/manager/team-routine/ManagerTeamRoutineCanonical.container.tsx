...
function RoutineComparisonBar({ label, value, color, icon: Icon }: { label: string; value: number | null; color: string; icon?: typeof Trophy }) { return <div className="flex items-center gap-3"><div className="flex w-40 shrink-0 items-center gap-2 text-sm text-muted-foreground">{Icon&&<Icon size={14}/>} {label}</div><div className="relative h-6 flex-1 overflow-hidden rounded-full bg-gray-100">{value!==null&&<div className={`flex h-full items-center justify-end rounded-full pr-2 text-xs font-bold text-white ${color}`} style={{width:`${Math.min(value,100)}%`}}>{value}%</div>}{value===null&&<span className="absolute inset-y-0 right-2 grid place-items-center text-xs font-bold text-muted-foreground">—</span>}</div></div> }
