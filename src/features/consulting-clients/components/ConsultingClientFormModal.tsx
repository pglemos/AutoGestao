  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="consulting-client-modal-title" onMouseDown={event => { if (event.currentTarget === event.target) props.onClose() }}>
      <section className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-5 shadow-xl">
        <h2 id="consulting-client-modal-title" className="text-xl font-bold text-gray-800">Novo cliente da consultoria</h2>
        ...
        <div className="mt-6 flex justify-end gap-2"><Button variant="managerSecondary" onClick={props.onClose} disabled={props.submitting}>Cancelar</Button><Button onClick={props.onSubmit} disabled={props.submitting}>{props.submitting ? 'Salvando...' : 'Criar cliente'}</Button></div>
      </section>
    </div>
  )
}
