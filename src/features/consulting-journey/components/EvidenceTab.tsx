            ...
            <article key={evidence.id} className="rounded-lg border border-border p-3">
              ...
              {controller.canReviewEvidence && evidence.status !== 'replaced' ? (
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <button type="button" onClick={() => { setReturningEvidenceId(evidence.id); setReturnFeedback(evidence.feedback || '') }} className="rounded-md bg-status-warning px-2.5 py-1.5 text-xs text-status-warning-foreground">Devolver</button>
                </div>
              ) : null}
              {returningEvidenceId === evidence.id ? (
                <div className="mt-3 rounded-lg border border-status-warning/30 bg-status-warning-surface p-3">
                  <label className="text-xs font-semibold text-status-warning-text" htmlFor={`feedback-${evidence.id}`}>Devolutiva obrigatória</label>
