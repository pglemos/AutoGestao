    ...
    <section className="rounded-xl border border-border bg-card p-4">
      ...
      {!active ? (
        <div className="mt-3 space-y-3">
          {!eligibility.eligible ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-3"><p className="text-sm font-medium text-amber-900">Pré-requisitos pendentes</p><ul className="mt-1 list-disc pl-5 text-xs text-amber-800">{eligibility.blockers.map(blocker => <li key={blocker}>{blocker}</li>)}</ul></div> : null}
      ...
      {request && controller.canReviewAnticipation && ['under_review', 'date_adjustment_requested'].includes(request.status) ? (
        <div className="mt-4 space-y-3 rounded-lg border border-border p-3">
          ...
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => void controller.reviewAnticipation(request.id, 'approved', reviewNote || null)} className="rounded-md bg-emerald-600 px-3 py-2 text-sm text-white">Aprovar</button><button type="button" disabled={!reviewNote.trim()} onClick={() => void controller.reviewAnticipation(request.id, 'date_adjustment_requested', reviewNote)} className="rounded-md bg-amber-600 px-3 py-2 text-sm text-white disabled:opacity-50">Pedir ajuste</button><button type="button" disabled={!reviewNote.trim()} onClick={() => void controller.reviewAnticipation(request.id, 'rejected', reviewNote)} className="rounded-md bg-destructive px-3 py-2 text-sm text-destructive-foreground disabled:opacity-50">Recusar</button></div>
        </div>
      ) : null}
    </section>
  )
