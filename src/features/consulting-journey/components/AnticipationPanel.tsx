    ...
    <section className="rounded-xl border border-border bg-card p-4">
      ...
      {request && controller.canReviewAnticipation && ['under_review', 'date_adjustment_requested'].includes(request.status) ? (
        <div className="mt-4 space-y-3 rounded-lg border border-border p-3">
          ...
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => void controller.reviewAnticipation(request.id, 'approved', reviewNote || null)} className="rounded-md bg-brand-primary px-3 py-2 text-sm text-white">Aprovar</button><button type="button" disabled={!reviewNote.trim()} onClick={() => void controller.reviewAnticipation(request.id, 'date_adjustment_requested', reviewNote)} className="rounded-md bg-status-warning px-3 py-2 text-sm text-white disabled:opacity-50">Pedir ajuste</button><button type="button" disabled={!reviewNote.trim()} onClick={() => void controller.reviewAnticipation(request.id, 'rejected', reviewNote)} className="rounded-md bg-destructive px-3 py-2 text-sm text-destructive-foreground disabled:opacity-50">Recusar</button></div>
        </div>
      ) : null}
    </section>
  )
