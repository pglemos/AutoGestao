    ...
    <div className="min-h-full bg-surface-alt text-foreground">
      ...
      {flash && (
        <div
          role="status"
          aria-live="polite"
          className={`fixed bottom-5 right-5 z-[var(--mx-z-toast)] flex max-w-sm items-center gap-2 rounded-2xl border bg-white px-4 py-3 text-body-sm font-semibold shadow-xl ${flash.kind === 'success' ? 'border-green-200 text-green-700' : 'border-status-error/30 text-status-error-text'}`}
        >
