    ...
    <PageCanvas as="div" width="dashboard" bottomClearance="navigation" className="flex min-h-full flex-col bg-surface-alt text-foreground">
      ...
      {flash && (
        <div
          role="status"
          aria-live="polite"
          className={`fixed bottom-5 right-5 z-[var(--mx-z-toast)] flex max-w-sm items-center gap-2 rounded-2xl border bg-white px-4 py-3 text-body-sm font-semibold shadow-xl ${flash.kind === 'success' ? 'border-brand-primary/30 text-brand-primary-hover' : 'border-status-error/30 text-status-error-text'}`}
        >
