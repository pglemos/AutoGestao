import { CalendarClock, Car, CircleDollarSign, History, Phone } from 'lucide-react'
...
export function FichaClienteSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={next => { if (!next) onClose() }}>
      <SheetContent side="right" className="w-full max-w-[var(--mx-overlay-size-lg)] bg-surface-alt p-0 shadow-2xl">
        <SheetHeader className="h-16 shrink-0 flex-row items-center justify-between border-b border-border bg-white px-5">
          <div className="min-w-0">
            <SheetTitle className="truncate text-h5 font-bold text-foreground">Ficha do cliente</SheetTitle>
            <SheetDescription className="truncate text-[12px] text-muted-foreground">Histórico comercial sem sair da Central</SheetDescription>
          </div>
        </SheetHeader>
