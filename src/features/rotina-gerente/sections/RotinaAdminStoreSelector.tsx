...
export function RotinaAdminStoreSelector({ lojas, onSelect }: Props) {
  return (
    <Card className="p-mx-10 md:p-14 border-none bg-white text-center space-y-mx-lg">
      <div className="w-mx-3xl h-mx-3xl rounded-2xl bg-brand-primary/10 flex items-center justify-center mx-auto">
        <Store size={48} className="text-status-success-text" />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-mx-sm max-w-3xl mx-auto pt-mx-lg">
        {lojas.map((store) => (
          <Button
            ...
            className="h-mx-2xl rounded-2xl font-bold uppercase tracking-widest text-xs border-border hover:border-brand-primary hover:bg-brand-primary/5 transition-all"
          >
            {store.name}
          </Button>
        ))}
