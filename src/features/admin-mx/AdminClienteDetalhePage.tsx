...
import {
  fetchClientActionPlanSummary,
  type ClientActionPlanSummary,
} from "./clientes/clientActionPlanContext";
...
} from "./clientes/enrollmentMutations";
import { buildProgramSummary } from "./clientes/programSummary";
import {
  buildClientJourney,
  clientVisitDisplayTitle,
  clientVisitStatusLabel,
  isCompletedClientVisit,
  isOverdueClientVisit,
  isClientVisitInContract,
} from "./clientes/clientJourney";
import {
  saveClientProgram,
  ensureImplementationOwnerFromAssignments,
  ensureContractStartFromCreatedAt,
  normalizeProgramModality,
  programModalityLabel,
  type ProgramDraft,
} from "./clientes/programMutations";
import {
  saveClientIdentification,
  ensureOnboardingCompleteWhenActive,
} from "./clientes/clientIdentificationMutations";
...
} from "./clientes/clientIdentification";
import {
  emptyStoreDraft,
  maskStoreCnpj,
  type StoreDraft,
} from "./clientes/storeForm";
import {
  deleteOrphanTestUnits,
  ensureOperationalUnitRows,
  fetchUnitOperatingHours,
  saveClientStore,
  type UnitRow,
} from "./clientes/storeMutations";
import {
  useAdminConsultingProducts,
  useAdminTeam,
} from "./hooks/useAdminMxLists";
...
} from "./clientes/mergeClientPeople";
import {
...
import { ClientDetailHeaderActions } from "./clientes/ClientDetailHeaderActions";
import { ClientOperationalSummary } from "./clientes/ClientOperationalSummary";
import {
  buildClientAttentionSummary,
  resolveClientNextAction,
  type ClientDetailNextAction,
} from "./clientes/clientDetailPresentation";
...
type DetailAreaKey =
  "area-visao" | "area-operacao" | "area-planejamento" | "area-dados";
type OperationAreaTab =
  "lojas" | "pessoas" | "jornada" | "implantacao" | "operacao";
type DataAreaTab = "dados" | "historico";

// Todas as áreas continuam disponíveis, mas a navegação é agrupada por intenção
// para não transformar a ficha em uma faixa de nove decisões concorrentes.
const TABS = [
...
const AREA_TABS = [
  {
    key: "area-visao" as const,
    label: "Visão Geral",
    controls: "client-detail-area-panel",
  },
  {
    key: "area-operacao" as const,
    label: "Operação",
    controls: "client-detail-area-panel",
  },
  {
    key: "area-planejamento" as const,
    label: "Planejamento",
    controls: "client-detail-area-panel",
  },
  {
    key: "area-dados" as const,
    label: "Dados e auditoria",
    controls: "client-detail-area-panel",
  },
];

const OPERATION_AREA_TABS = [
  { key: "lojas" as const, label: "Empresa e Lojas", controls: "client-detail-area-panel" },
  { key: "pessoas" as const, label: "Pessoas e Acessos", controls: "client-detail-area-panel" },
  { key: "jornada" as const, label: "Programa e Jornada", controls: "client-detail-area-panel" },
  { key: "implantacao" as const, label: "Implantação", controls: "client-detail-area-panel" },
  {
    key: "operacao" as const,
    label: "Módulos e configurações",
    controls: "client-detail-configuration-panel",
  },
];

const DATA_TABS = [
  { key: "dados" as const, label: "Dados", controls: "client-detail-data-panel" },
  { key: "historico" as const, label: "Histórico e Auditoria", controls: "client-detail-data-panel" },
];
...
const PLANNING_TABS = [
  {
    ...
    controls: "client-detail-planning-panel",
  },
  {
    key: "plano-acao" as const,
    label: "Plano de Ação",
    controls: "client-detail-planning-panel",
  },
];
...
const OPERATION_TABS = [
  {
    key: "modulos" as const,
    label: "Módulos e acessos",
    controls: "client-detail-configuration-panel",
  },
  {
...
function resolveDetailArea(tab: ClientTab): DetailAreaKey {
  if (tab === "visao") return "area-visao";
  if (tab === "planejamento") return "area-planejamento";
  if (tab === "dados" || tab === "historico") return "area-dados";
  return "area-operacao";
}

function defaultTabForArea(area: DetailAreaKey): ClientTab {
  if (area === "area-visao") return "visao";
  if (area === "area-planejamento") return "planejamento";
  if (area === "area-dados") return "dados";
  return "lojas";
}
...
function resolveInitialDetailTab(requestedTab: string | null): {
  ...
  if (requestedTab === "estrategico")
    return {
      tab: "planejamento",
      planningTab: "estrategico",
      operationTab: "modulos",
    };
  if (requestedTab === "plano-acao")
    return {
      tab: "planejamento",
      planningTab: "plano-acao",
      operationTab: "modulos",
    };
  if (requestedTab === "modulos")
    return {
      tab: "operacao",
      planningTab: "estrategico",
      operationTab: "modulos",
    };
  if (requestedTab === "configuracoes")
    return {
      tab: "operacao",
      planningTab: "estrategico",
      operationTab: "configuracoes",
    };
  if (requestedTab === "historico")
    return {
      tab: "historico",
      planningTab: "estrategico",
      operationTab: "modulos",
    };
  const tab = TABS.some((entry) => entry.key === requestedTab)
    ? (requestedTab as ClientTab)
    : "visao";
...
function formatDateTime(value: string | null | undefined) {
  ...
  return Number.isNaN(date.getTime())
    ? "Atualização ainda não concluída"
...
function formatClientStatus(value: string | null | undefined) {
  ...
  return value?.trim() || "Status não informado";
}

function internalRoleLabel(value: string | null | undefined) {
  const labels: Record<string, string> = {
    administrador_geral: "Administrador Geral",
    administrador_mx: "Administrador MX",
    consultor_mx: "Consultor MX",
  };
  return labels[value ?? ""] ?? "Equipe interna MX";
}
...
export function AdminClienteDetalhePage() {
  ...
  const { client, loading, enriching, error, refetch } =
  ...
  const [planningTab, setPlanningTab] = useState<PlanningTab>(
    initialTabState.planningTab,
  );
  const [operationTab, setOperationTab] = useState<OperationTab>(
    initialTabState.operationTab,
  );
  ...
  const [actionPlanSummary, setActionPlanSummary] =
    useState<ClientActionPlanSummary | null>(null);
  ...
  const [personStores, setPersonStores] = useState<
    Array<{ id: string; name: string; parent_loja_id?: string | null }>
  >([]);
  const [personsReady, setPersonsReady] = useState(false);
  const [personModal, setPersonModal] = useState(false);
  const [personPrefill, setPersonPrefill] =
    useState<Partial<PersonAccessDraft> | null>(null);
  ...
  const [strategicPlanReadiness, setStrategicPlanReadiness] =
    useState<ClientReadinessInput["strategic_plan_ready"]>(null);
  const [strategicPlanCycleId, setStrategicPlanCycleId] = useState<
    string | null
  >(null);
  const [strategicPlanYear, setStrategicPlanYear] = useState(() =>
  ...
  const loadUnits = useCallback(async () => {
    ...
    if (persisted.created) {
      const { data: refreshed } = await supabase
        ...
        .select("*")
        .eq("client_id", client.id);
      cadastro = ((refreshed ?? []) as UnitRow[]).filter(
        (unit) => !isOrphanTestUnit(unit),
      );
      void refetch();
    }
  ...
  const loadPersons = useCallback(async () => {
    ...
    const {
      ...
      stores,
      error: personsError,
    } = await fetchClientPersons(client.id);
    if (personsError) {
  ...
  const loadStrategicPlan = useCallback(async () => {
    ...
    setStrategicPlanReadiness({
      ...
      total:
        rosterCodes.length || card.indicadoresComMeta + card.metasPendentes,
  ...
  const handleStrategicPlanCycleChange = useCallback(
    (cycleId: string) => {
  ...
  const loadActionPlanSummary = useCallback(async () => {
    ...
    const result = await fetchClientActionPlanSummary(
      client.id,
      client.primary_store_id,
    );
    setActionPlanError(result.error);
    setActionPlanSummary(result.summary);
  }, [client?.id, client?.primary_store_id]);

  useEffect(() => {
    void loadActionPlanSummary();
  }, [loadActionPlanSummary, actionPlanRefreshKey]);
  ...
  const checks = useMemo(() => {
    ...
    return buildClientReadiness({
      ...
      program_template_key:
        (client as { program_template_key?: string | null })
          .program_template_key ?? null,
      modality: client.modality ?? null,
      cnpj: client.cnpj ?? null,
      contract_start_date:
        (client as { contract_start_date?: string | null })
          .contract_start_date ?? null,
      implementation_owner_id:
        (client as { implementation_owner_id?: string | null })
  ...
  useEffect(() => {
    if (
      !client?.id ||
      ownerMasterResolution.status !== "VALID" ||
      !ownerMasterResolution.person
    )
      return;
    const hasPrimary = (client.contacts ?? []).some(
      (contact) => contact.is_primary && (contact.name ?? "").trim(),
    );
    if (hasPrimary || syncingPrimaryContact.current) return;
    syncingPrimaryContact.current = true;
    void ensurePrimaryContactFromDonoMaster(
      client.id,
      ownerMasterResolution.person,
    ).then((result) => {
  ...
  useEffect(() => {
    if (
      ...
      return;
    const hasResponsible = (client.assignments ?? []).some(
      (item) => item.active !== false && item.assignment_role === "responsavel",
    );
  ...
  useEffect(() => {
    ...
    const done = Boolean(
      (client as { onboarding_completed?: boolean | null })
  ...
  const correctDonoMaster = async () => {
    ...
    if (
      ownerMasterResolution.status === "OWNER_WITHOUT_MASTER" &&
      donos.length === 1
    ) {
    ...
    if (
      ownerMasterResolution.status === "INACTIVE" &&
      ownerMasterResolution.person
    ) {
      const inactive = persons.find(
        (person) => person.id === ownerMasterResolution.person?.id,
      );
      if (inactive) {
        ...
        setPersonPrefill(
          personToAccessDraft({
            ...inactive,
            papeis: inactive.papeis.includes("DONO")
              ? inactive.papeis
              : [...inactive.papeis, "DONO"],
            is_dono_master: true,
          }),
        );
        setPersonModal(true);
  ...
  const correctReadiness = (key: string) => {
    ...
    if (key === "contrato" || key === "responsavel-mx")
  ...
  const overdueVisitIds = useMemo(
    () =>
      visits
        .filter(
          (visit) =>
            isClientVisitInContract(visit.visit_number, totalVisits) &&
            isOverdueClientVisit(visit),
        )
        .map((visit) => visit.id)
  ...
  const completeOverdue = async () => {
    ...
    try {
      ...
      toast.success(
        `${quantity(result.updated, "encontro")} ${result.updated === 1 ? "marcado" : "marcados"} como ${result.updated === 1 ? "realizado" : "realizados"}.`,
      );
      await refetch();
    } finally {
  ...
  const portfolioStatus = useMemo(() => {
    ...
    return canonicalPortfolioStatus({
      ...
      product_name: client.product_name ?? null,
      assignments:
        client.assignments?.filter((assignment) => assignment.active).length ??
        0,
      modulesEnabled:
        client.modules?.filter((module) => module.enabled !== false).length ??
        0,
      visitsDone: journey.completedVisits,
      visitsTotal: totalVisits,
    });
  }, [client, journey.completedVisits, totalVisits]);
  const clientIsActive = client ? isActive(client) : false;
  const portfolioStatusLabel = portfolioStatus
    ? PORTFOLIO_STATUS_LABEL[portfolioStatus]
    : formatClientStatus(client?.status);
  const journeyProgressPct =
    totalVisits > 0
      ? Math.round((journey.completedVisits / totalVisits) * 100)
      : 0;
  const showValidateActivate = useMemo(() => {
    if (!client) return false;
    if (!clientIsActive) return true;
    if (
      portfolioStatus === "em_configuracao" ||
      portfolioStatus === "prontos_para_ativar"
    )
      return true;
    return !summary.canActivate;
  }, [client, clientIsActive, portfolioStatus, summary.canActivate]);
  const activeArea = resolveDetailArea(tab);
  const activeOperationAreaTab: OperationAreaTab =
    tab === "operacao" ||
    tab === "lojas" ||
    tab === "pessoas" ||
    tab === "jornada" ||
    tab === "implantacao"
      ? tab
      : "lojas";
  const activeDataAreaTab: DataAreaTab =
    tab === "historico" ? "historico" : "dados";
  const integrity = useMemo(
    () => dataIntegritySummary(health.sources),
    [health.sources],
  );
  const dataHealth: "loading" | "error" | "ready" = health.error
    ? "error"
    : health.loading
      ? "loading"
      : "ready";
  const attention = useMemo(
    () =>
      buildClientAttentionSummary({
        ...
        overdueVisitCount: journey.overdueVisits,
        overdueActionCount: actionPlanError
          ? null
          : actionPlanSummary?.atrasadas,
        actionPlanError: Boolean(actionPlanError),
        missingDataCount:
          health.loading || health.error ? null : integrity.vazios,
        staleDataCount:
          health.loading || health.error ? null : integrity.desatualizados,
        dataHealth,
      }),
    [
      actionPlanError,
      ...
      health.error,
      health.loading,
      integrity.desatualizados,
      integrity.vazios,
      journey.overdueVisits,
  ...
  const nextAction = useMemo(
    () =>
      resolveClientNextAction({
        requiresActivation: showValidateActivate,
        clientIsActive,
        blockerCount: summary.blockers.length,
        overdueVisitCount: journey.overdueVisits,
        overdueActionCount: actionPlanError
        ...
        staleDataCount:
          health.loading || health.error ? null : integrity.desatualizados,
        dataHealth,
        strategicPlanPendingCount: strategicPlanReadiness?.pending,
        warningCount: summary.warnings.length,
        journeyIncomplete: journey.completedVisits < totalVisits,
        totalVisits,
      }),
    [
      actionPlanError,
      ...
      journey.overdueVisits,
      showValidateActivate,
      strategicPlanReadiness?.pending,
      summary.blockers.length,
      summary.warnings.length,
      totalVisits,
    ],
  );
  const handleNextAction = useCallback((action: ClientDetailNextAction) => {
    if (action.key === "ativacao") {
      setActivationOpen(true);
      return;
    }
    if (action.key === "jornada") {
      setTabState("jornada");
      return;
    }
    if (action.key === "implantacao") {
      setTabState("implantacao");
      return;
    }
    if (action.key === "dados") {
      setTabState("dados");
    ...
    setPlanningTab(action.key === "plano-acao" ? "plano-acao" : "estrategico");
    setTabState("planejamento");
  }, []);
  const refreshDetail = useCallback(async () => {
    ...
    try {
      ...
      setActionPlanRefreshKey((current) => current + 1);
  ...
  useEffect(() => {
    if (!client?.id || enriching || health.loading || lastUpdatedAt) return;
    setLastUpdatedAt(new Date().toISOString());
  }, [client?.id, enriching, health.loading, lastUpdatedAt]);
  ...
  const operationalStatus = useMemo<{
    ...
    if (summary.blockers.length > 0) {
      return {
        tone: "danger",
        label: "Ativação bloqueada",
        description: `${quantity(summary.blockers.length, "item obrigatório", "itens obrigatórios")} ${summary.blockers.length === 1 ? "precisa" : "precisam"} de correção antes de liberar a operação.`,
      };
    }
    if (dataHealth === "error") {
      return {
        ...
        description:
          "A ficha carregou, mas a integridade das fontes não pôde ser confirmada.",
      };
    }
    if (actionPlanError) {
      return {
        ...
        description:
          "A operação pode ser acompanhada, mas o Plano de Ação precisa ser carregado novamente.",
      };
    }
    if (attention.count > 0) {
      return {
        tone: "warning",
        label: clientIsActive
          ? "Operação ativa com atenção"
          : "Configuração pendente",
        description:
          "A operação tem sinais que merecem uma decisão antes de ser considerada em dia.",
      };
    }
    if (clientIsActive) {
      return {
        ...
        description:
          "Não há pendências operacionais identificadas nesta leitura.",
      };
    }
    return {
      tone: "info",
      ...
      description:
        "A ficha está disponível para concluir a configuração e acompanhar a operação.",
    };
  }, [
    actionPlanError,
    attention.count,
  ...
  const responsibleConsultant = useMemo(() => {
    const primary = client?.assignments?.find(
      (a) => a.active && a.assignment_role === "responsavel",
    )?.user?.name;
      ...
      buildProgramSummary({
        ...
        contract_start_date:
          (client as { contract_start_date?: string | null })
            ?.contract_start_date ?? null,
        contract_end_date:
          (client as { contract_end_date?: string | null })
  ...
  const selectedProduct = useMemo(
    () =>
      products.rows.find(
        (product) => product.program_key === client?.program_template_key,
      ) ?? null,
  ...
  const consultingCurrentLine = useMemo(() => {
    if (totalVisits > 0 && journey.completedVisits >= totalVisits)
    ...
    const open = visits.find((visit) => {
      const status = String(visit.status ?? "")
        ...
        .toLowerCase();
      return (
        !isCompletedClientVisit(visit.status) &&
        status !== "cancelada" &&
        status !== "cancelado"
      );
    });
    ...
    return current
      ? `Encontro atual: ${clientVisitDisplayTitle(current)}`
      : "Sem encontro na jornada";
  ...
  const programInitialDraft = useMemo<ProgramDraft>(() => {
    ...
    const responsible =
      assignments.find((a) => a.active && a.assignment_role === "responsavel")
        ?.user_id ?? "";
    const auxiliaries = assignments
      .filter((a) => a.active && a.assignment_role !== "responsavel")
      .map((a) => a.user_id);
    return {
      ...
      program_template_key:
        (client as { program_template_key?: string | null })
          ?.program_template_key ?? "",
      modality:
        normalizeProgramModality(client?.modality) || client?.modality || "",
      contract_start_date:
        (client as { contract_start_date?: string | null })
          ?.contract_start_date ||
      ...
      implementation_owner_id:
        (client as { implementation_owner_id?: string | null })
          ?.implementation_owner_id || responsible,
  ...
  const identityInitial = useMemo(() => {
    ...
    return buildClientIdentificationDraft({
      ...
      businessPhase:
        (client as { business_phase?: string | null }).business_phase ?? null,
      contractEndDate:
        (client as { contract_end_date?: string | null }).contract_end_date ??
        null,
    });
  ...
  const implementationOwnerName =
    team.rows.find(
      (member) =>
        member.id ===
        (client as { implementation_owner_id?: string | null } | null)
          ?.implementation_owner_id,
    )?.name ?? responsibleConsultant;
  const headerLocation = [identityUnit?.city, identityUnit?.state]
    .filter(Boolean)
    .join(", ");
  ...
  const onboardingStep =
    (client as { onboarding_step?: number | null })?.onboarding_step ?? 1;
  const onboardingCompleted =
    (client as { onboarding_completed?: boolean | null })
      ?.onboarding_completed ?? false;
  const plannedStartDate = formatDate(
    resolveClientPlannedStartDate(client),
    "Início não definido",
  );
      ...
      if (updateError) {
        toast.error(
          updateError.code === "23505"
            ? "Esta loja já tem um cliente ativo."
            : updateError.message,
        );
        return;
      }
  ...
  const submitStore = async (
    draft: StoreDraft,
    hours: Parameters<typeof saveClientStore>[2],
  ) => {
    if (!client?.id || !supabaseUser) return;
    setSavingStore(true);
    try {
      const { error: saveError } = await saveClientStore(
        client.id,
        draft,
        hours,
        supabaseUser.id,
      ...
      toast.success(
        draft.id ? "Loja atualizada." : "Loja criada com horário padrão MX.",
      );
  ...
  const submitIdentity = async (
    draft: Parameters<typeof saveClientIdentification>[0]["draft"],
  ) => {
    if (!client?.id || !supabaseUser) return;
  ...
  const submitLink = async (
    draft: Parameters<typeof createEnrollmentLink>[3],
  ) => {
    if (!client?.id || !client.slug || !supabaseUser) return null;
    setSavingLink(true);
    try {
      const result = await createEnrollmentLink(
        client.id,
        client.slug,
  ...
  return (
    ...
    >
      <div className="w-full space-y-5">
        <MxModuleHeader
          icon={Building2}
          eyebrow="Painel interno MX"
          title={client?.name ?? "Cliente"}
          description={
            client ? (
              <>
                <span className="block">{`${client.legal_name || "Razão social não informada"} · ${client.product_name || "Produto não configurado"}`}</span>
                {headerMeta.location ||
                headerMeta.comercial ||
                headerMeta.implantacao ? (
                  <span className="mt-1 block">
                    {[
                      headerMeta.location,
                      headerMeta.comercial
                        ? `Comercial: ${headerMeta.comercial}`
                        : null,
                      headerMeta.implantacao
                        ? `Implantação: ${headerMeta.implantacao}`
                        : null,
                    ]
                      .filter(Boolean)
                      .join(" · ")}
                ...
                <span className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                  <span className="font-semibold text-status-success-text">
                    Acesso: {internalRoleLabel(role)}
                  </span>
                  <span aria-hidden="true">·</span>
                  <span>Operação: {portfolioStatusLabel}</span>
                  <span aria-hidden="true">·</span>
                  <span>
                    {lastUpdatedAt
                      ? `Última leitura: ${formatDateTime(lastUpdatedAt)}`
                      : "Última leitura: aguardando carregamento"}
                  </span>
                </span>
              </>
            ) : (
              "Área interna MX para administração, operação e consultoria do cliente."
            )
          }
          actions={
            <>
              ...
              {client && showValidateActivate ? (
                ...
                >
                  ...
                  {clientIsActive ? "Revisar ativação" : "Validar e Ativar"}
                </Button>
              ) : null}
              {client ? (
                <ClientDetailHeaderActions
                  ...
                  onContinueOnboarding={
                    ...
                    String(client.status ?? "").toLowerCase() !== "ativo"
                      ? () =>
                          navigate(
                            `/clientes/novo?continue=${encodeURIComponent(client.id)}`,
                          )
                      : undefined
                  }
                  onOpenStrategicPlan={() => void openCurrentStrategicPlan()}
                  onOpenActionPlan={() => {
                    setPlanningTab("plano-acao");
                    setTab("planejamento");
                  }}
                  onOpenConsulting={() =>
                    navigate(
                      `/clientes/${encodeURIComponent(client.slug || client.id)}/consultoria`,
                    )
                  }
                />
              ) : null}
        ...
        {loading && !client ? (
          <MxLoadingState label="Carregando cliente" />
        ) : error ? (
          ...
          <>
            {enriching ? (
              <MxStatusBanner tone="info">
                Atualizando jornada, contatos e módulos do cliente…
              </MxStatusBanner>
            ) : null}
            <ClientOperationalSummary
              ...
              completedValue={
                totalVisits > 0
                  ? `${journey.completedVisits}/${totalVisits}`
                  : `${summary.completed}/${summary.total}`
              }
              completedDetail={
                totalVisits > 0
                  ? `${journeyProgressPct}% da jornada contratada`
                  : "itens de prontidão validados"
              }
              attentionValue={
                health.loading
                  ? "Em verificação"
              ...
              onNextAction={() => handleNextAction(nextAction)}
            />
            <MxMetricGrid>
              <MxMetricCard
                title="Lojas"
                value={displayUnits.length}
                detail="Matriz e filiais operacionais"
                icon={Building2}
                tone="info"
              />
              <MxMetricCard
                ...
                detail="Acessos e vínculos de loja"
                icon={UserPlus}
                tone="violet"
              />
              <MxMetricCard
                title="Encontros concluídos"
                value={journey.completedVisits}
                detail={
                  totalVisits > 0
                    ? `de ${totalVisits} previstos`
                    : "Sem jornada contratada"
                }
                icon={Clock}
                tone="info"
              />
              <MxMetricCard
                ...
                detail={
                  totalVisits > 0
                    ? `${journey.completedVisits}/${totalVisits} encontros`
                    : "Jornada não contratada"
                }
                icon={BriefcaseBusiness}
                tone={journeyProgressPct >= 100 ? "success" : "brand"}
              />
            </MxMetricGrid>

            <section
              data-mx-client-detail-navigation=""
              aria-label="Navegação da ficha do cliente"
            >
              <div className="hidden rounded-2xl border border-border-subtle bg-white p-3 shadow-sm medium:block">
                <div className="px-2 pt-1">
                  <p className="text-sm font-semibold text-foreground">
                    Áreas da ficha
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Organizadas por intenção para manter a próxima decisão
                    visível.
                  </p>
                </div>
                <TabNav
                  ...
                  onTabChange={(area) => setTab(defaultTabForArea(area))}
                  ariaLabel="Áreas da ficha do cliente"
                  className="mb-0 mt-3"
                />
                {activeArea === "area-operacao" ? (
                  <div className="mt-3 border-t border-border-subtle px-2 pt-3">
                    <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                      Detalhes da operação
                    </p>
                    <TabNav
                      tabs={OPERATION_AREA_TABS}
                      activeTab={activeOperationAreaTab}
                      onTabChange={(next) => setTab(next)}
                      ariaLabel="Detalhes da operação do cliente"
                      scrollable
                      className="mb-0"
                    />
                    {activeOperationAreaTab === "operacao" ? (
                      <div className="mt-3 border-t border-border-subtle pt-3">
                        ...
                        <TabNav
                          ...
                          ariaLabel="Seções de configuração do cliente"
                          className="mb-0"
                        />
                      </div>
                    ) : null}
                ...
                {activeArea === "area-planejamento" ? (
                  <div className="mt-3 border-t border-border-subtle px-2 pt-3">
                    ...
                    <TabNav
                      ...
                      ariaLabel="Detalhes do planejamento do cliente"
                      className="mb-0"
                    />
                  </div>
                ) : null}
                {activeArea === "area-dados" ? (
                  <div className="mt-3 border-t border-border-subtle px-2 pt-3">
                    <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                    ...
                    <TabNav
                      tabs={DATA_TABS}
                      activeTab={activeDataAreaTab}
                      onTabChange={(next) => setTab(next)}
                      ariaLabel="Dados e auditoria do cliente"
                      className="mb-0"
                    />
                  </div>
                ) : null}
              ...
              <div className="space-y-3 medium:hidden">
                <Select
                  id="client-detail-area"
                  label="Área da ficha"
                  value={activeArea}
                  onChange={(event) =>
                    setTab(
                      defaultTabForArea(event.target.value as DetailAreaKey),
                    )
                  }
                >
                  {AREA_TABS.map((area) => (
                ...
                {activeArea === "area-operacao" ? (
                  <Select
                    id="client-detail-operation-area"
                    label="Detalhe da operação"
                    value={activeOperationAreaTab}
                    onChange={(event) =>
                      setTab(event.target.value as OperationAreaTab)
                    }
                  >
                    {OPERATION_AREA_TABS.map((operation) => (
                ...
                activeOperationAreaTab === "operacao" ? (
                  <Select
                    id="client-detail-configuration-area"
                    label="Seção de configuração"
                    value={operationTab}
                    onChange={(event) =>
                      setOperationTab(event.target.value as OperationTab)
                    }
                  >
                    {OPERATION_TABS.map((operation) => (
                      <option key={operation.key} value={operation.key}>
                        {operation.label}
                      </option>
                    ))}
                ...
                {activeArea === "area-planejamento" ? (
                  <Select
                    id="client-detail-planning-area"
                    label="Detalhe do planejamento"
                    value={planningTab}
                    onChange={(event) =>
                      setPlanningTab(event.target.value as PlanningTab)
                    }
                  >
                    {PLANNING_TABS.map((planning) => (
                      <option key={planning.key} value={planning.key}>
                        {planning.label}
                      </option>
                    ))}
                  </Select>
                ...
                {activeArea === "area-dados" ? (
                  <Select
                    id="client-detail-data-area"
                    label="Detalhe da auditoria"
                    value={activeDataAreaTab}
                    onChange={(event) =>
                      setTab(event.target.value as DataAreaTab)
                    }
                  >
                    {DATA_TABS.map((data) => (
            ...
            >
                ...
                <div className="space-y-5">
                  <MxSectionCard>
                    <MxSectionHeader
                      ...
                      description="Fase, estrutura e progresso do onboarding."
                    />
                    <div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4">
                      {[
                        [
                          "Fase empresarial",
                          clientBusinessPhaseLabel(
                            (client as { business_phase?: string | null })
                              .business_phase,
                          ),
                        ],
                        [
                          "Estrutura",
                          clientStructureDisplay(
                            (client as { structure_type?: string | null })
                              .structure_type,
                          ),
                        ],
                        [
                  ...
                  <MxSectionCard>
                    ...
                    <div className="grid gap-3 p-5 sm:grid-cols-3">
                      {[
                        ["Nome", primaryContact?.name || "Nome não informado"],
                        [
                          "Telefone",
                  ...
                  <MxSectionCard>
                    <MxSectionHeader
                      title="Entrega da Consultoria"
                      description="Plano estratégico, plano de ação e consultoria do programa contratado."
                    />
                    <div className="grid gap-3 p-5 lg:grid-cols-3">
                      <div className="rounded-xl border border-border bg-surface-alt p-4">
                        ...
                        <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                          ...
                          <div>
                            Indicadores com meta:{" "}
                            {strategicPlanReadiness
                              ? (strategicPlanReadiness.indicadoresComMeta ?? 0)
                              : "Plano ainda não criado"}
                          </div>
                          <div>
                            Metas publicadas:{" "}
                            <span className="font-semibold text-status-success-text">
                              {strategicPlanReadiness
                                ? strategicPlanReadiness.ready
                                : "Aguardando leitura"}
                            </span>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <span className="font-semibold text-foreground">
                              {strategicPlanReadiness
                                ? PLAN_CYCLE_STATUS_LABEL[
                                    strategicPlanReadiness.cycleStatus
                                  ]
                                : "Plano ainda não criado"}
                            </span>
                          </div>
                        ...
                        <Button
                          ...
                          size="sm"
                          className="mt-4"
                          onClick={() => void openCurrentStrategicPlan()}
                          disabled={creatingStrategicPlan}
                        >
                          Abrir Plano
                        </Button>
                      </div>
                      <div className="rounded-xl border border-border bg-surface-alt p-4">
                        ...
                        <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                          ...
                          <div>
                            ...
                            {actionPlanError
                              ? "Leitura indisponível"
                              : (actionPlanSummary?.naoIniciadas ??
                                "Aguardando leitura")}
                          </div>
                          <div>
                            Em andamento:{" "}
                      ...
                      <div className="rounded-xl border border-border bg-surface-alt p-4">
                        ...
                        <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                          ...
                          <div>
                            ...
                            {totalVisits > 0
                              ...
                              : visits.length}
                            {journey.overdueVisits
                              ? ` · ${journey.overdueVisits} atrasada(s)`
                              : ""}
                          </div>
                        </div>
                        <Button
                        ...
                        >
                          <Link
                            to={`/clientes/${encodeURIComponent(client.slug || client.id)}/consultoria`}
                          >
                            Abrir Consultoria
                          </Link>
                        </Button>
              ...
              {tab === "lojas" ? (
                <MxSectionCard>
                    <MxSectionHeader
                      ...
                      description={`${quantity(displayUnits.length, "loja operacional")} — matriz e filiais.`}
                    actions={
                      <Button
                        size="sm"
                        onClick={() =>
                          setStoreModal({ open: true, initial: null })
                        }
                      >
                        <Plus size={16} />
                        Adicionar Loja
                  ...
                  <div className="space-y-3 p-5">
                    {displayUnits.length ? (
                        ...
                        >
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex min-w-0 flex-wrap items-center gap-2">
                              ...
                              {unit.store_type === "matriz" ? (
                                <span className="rounded-full bg-primary/10 px-2 py-0.5 text-caption font-medium text-primary">
                                  Matriz
                                </span>
                              ) : null}
                              {unit.store_type === "filial" ? (
                                <span className="rounded-full bg-status-info-bg px-2 py-0.5 text-caption font-medium text-status-info-text">
                                  Filial
                                </span>
                              ) : null}
                              {unit.is_primary ? (
                                <span className="rounded-full bg-surface-alt px-2 py-0.5 text-caption font-medium text-muted-foreground">
                                  Principal
                                </span>
                              ) : null}
                              {unit.status === "inativa" ? (
                                <span className="rounded-full bg-status-warning-surface px-2 py-0.5 text-caption font-medium text-status-warning-text">
                                  Inativa
                                </span>
                              ) : null}
                            </div>
                                    ...
                                    onClick={() =>
                                      setStoreModal({
                                        ...
                                        initial: {
                                          ...
                                          address_street:
                                            unit.address_street ?? "",
                                          address_city: unit.city ?? "",
                                          address_state: unit.state ?? "",
                          ...
                          {hoursUnitId === unit.id ? (
                            <div className="mt-3 border-t border-border pt-3">
                              <StoreOperatingHoursEditor
                                unitId={unit.id}
                                unitName={unit.name}
                                origin="Visão 360 — Empresa e Lojas"
                              />
                            </div>
                          ) : null}
                        </div>
                      ...
                      <MxEmptyState
                        ...
                        description="Cadastre as lojas do cliente para orientar a jornada."
                      />
                    )}
                  </div>
                </MxSectionCard>
                ...
                <div className="space-y-5">
                  <DonoMasterCard
                    ...
                    onEdit={() => {
                      ...
                      const row = master
                        ? persons.find((person) => person.id === master.id)
                        : null;
                      if (row) openPersonEdit(row);
                      else
                        toast.info(
                          "Cadastre ou escolha o Dono Master para editar.",
                        );
                    }}
                  />
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl border border-border p-4">
                      ...
                      <p className="mb-3 text-xs text-muted-foreground">
                        Cadastre usuários manualmente com papel, Loja e visão
                        padrão.
                      </p>
                      <Button
                        size="sm"
                    ...
                    <div className="rounded-xl border border-border p-4">
                      <div className="mb-2 flex items-center gap-2">
                        <Link2 size={16} className="text-status-info-text" />
                        <h4 className="text-sm font-medium text-foreground">
                          Autocadastro por Link
                        </h4>
                      </div>
                      <p className="mb-3 text-xs text-muted-foreground">
                        Gere um link para que a equipe do cliente se cadastre.
                      </p>
                      <Button
                        size="sm"
                        variant="outline"
                  ...
                  <MxSectionCard>
                    <MxSectionHeader
                      title={`Usuários (${persons.length})`}
                      description="Equipe por loja: cada filial tem gerente e vendedores próprios."
                    />
                    <div className="p-5">
                      {persons.length ? (
                            ...
                            <div key={group.storeId} className="space-y-2">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {group.kind === "filial" ? (
                                  ...
                                  <span className="text-xs text-muted-foreground">
                                  {group.gerenteNome
                                    ? `Gerente: ${group.gerenteNome}`
                                    : "Sem gerente"}
                                  {" · "}
                                  {quantity(group.people.length, "pessoa")}
                                </span>
                              </div>
                              {group.people.map((person) => (
                                <div
                                  key={`${group.storeId}:${person.id}`}
                                  className="flex flex-col gap-3 rounded-lg border border-border p-3 sm:flex-row sm:items-center sm:justify-between"
                                >
                                  <div className="flex min-w-0 items-center gap-3">
                                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-surface-alt text-xs font-bold text-foreground">
                                      {person.nome.charAt(0) || "?"}
                                    </div>
                                    <div className="min-w-0">
                                      <div className="flex items-center gap-2 text-sm font-medium text-foreground">
                                        {person.nome}
                                        {person.is_dono_master ? (
                                          <Crown
                                            size={12}
                        ...
                        <MxEmptyState
                          ...
                          description="Adicione usuários diretamente ou gere um link de autocadastro."
                        />
                      )}
                    </div>
                  </MxSectionCard>
                  {links.length ? (
                    <MxSectionCard>
                      <MxSectionHeader
                        title={`Links de autocadastro (${links.length})`}
                        description="Validade e usos de cada link gerado."
                      />
                      <div className="p-5">
                        <p className="mb-2 flex items-center gap-1.5 text-xs text-muted-foreground medium:hidden">
                          <MoveHorizontal size={14} aria-hidden="true" />
                          Deslize horizontalmente para consultar validade e
                          status.
                        </p>
                        <MxTableSurface>
                          <Table
                            className="min-w-[560px]"
                            aria-label="Links de autocadastro do cliente"
                          >
                            <TableHeader>
                              <TableRow>
                              ...
                              {links.map((link) => (
                                <TableRow key={link.id}>
                                  <TableCell className="font-semibold text-foreground">
                                    {link.nome_interno || link.perfil_acesso}
                                  </TableCell>
                                  <TableCell>
                                    {link.validade_dias} dias
                                  </TableCell>
                                  <TableCell>
                                    {link.usos_consumidos}/{link.limite_usos}
                                  </TableCell>
              ...
              {tab === "jornada" ? (
                <div className="space-y-5">
                  <ProgramCard
                    ...
                    visitRule={visitRule}
                    onEditProgram={() => setProgramModalOpen(true)}
                  />
                  <MxSectionCard>
                    <MxSectionHeader
                      title="Jornada de encontros"
                      description={`${journey.completedVisits} concluídos · ${Math.max(0, journey.totalVisits - journey.completedVisits)} restantes${journey.overdueVisits ? ` · ${quantity(journey.overdueVisits, "encontro atrasado", "encontros atrasados")}` : ""}`}
                      actions={
                        overdueVisitIds.length ? (
                          <Button
                            size="sm"
                            onClick={() => void completeOverdue()}
                            disabled={completingOverdue}
                          >
                            {completingOverdue
                              ? "Registrando..."
                              : `Marcar ${quantity(overdueVisitIds.length, "visita atrasada", "visitas atrasadas")} como ${overdueVisitIds.length === 1 ? "realizada" : "realizadas"}`}
                          </Button>
                        ) : undefined
                      }
                    />
                    <div className="space-y-4 p-5">
                      {health.presence ? (
                        <MxStatusBanner
                          tone={
                            health.presence.disponiveis === 0
                              ? "warning"
                              : "info"
                          }
                        >
                          {health.presence.contratadas === null
                            ? `Encontros presenciais: ${quantity(health.presence.usadas, "encontro marcado")} · ${visitRule.detail}`
                            : `Encontros presenciais: ${health.presence.usadas} de ${health.presence.contratadas} · ${health.presence.disponiveis === null ? "saldo indisponível" : quantity(health.presence.disponiveis, "encontro disponível")} ${health.presence.minimas != null ? `· mínimo contratado ${health.presence.minimas}` : ""}.`}
                        </MxStatusBanner>
                      ) : null}
                      {visits.length ? (
                        <>
                          <p className="mb-2 flex items-center gap-1.5 text-xs text-muted-foreground medium:hidden">
                            <MoveHorizontal size={14} aria-hidden="true" />
                            Deslize horizontalmente para consultar consultor,
                            data e status.
                          </p>
                          <MxTableSurface>
                            <Table
                              className="min-w-[640px]"
                              aria-label="Jornada de encontros do cliente"
                            >
                              <TableHeader>
                                <TableRow>
                              ...
                              <TableBody>
                                {visits.map((visit) => {
                                  const consultantName =
                                    ...
                                    team.rows.find(
                                      (member) =>
                                        member.id === visit.consultant_id,
                                    )?.name ??
                                    "Consultor não atribuído";
                                  const modality =
                                    String(visit.modality || "")
                                      .trim()
                                      .toUpperCase() ||
                        ...
                        <MxEmptyState
                          title="Jornada não iniciada"
                          description="Nenhum encontro registrado para este cliente."
                        />
                      )}
                    </div>
                  </MxSectionCard>
              ...
              {tab === "implantacao" ? (
                <ClientImplantacaoTab
                  bars={buildProgressBars({
                    ...
                    modulesEnabled: (client.modules ?? []).filter(
                      (module) => module.enabled !== false,
                    ).length,
                    modulesTotal: (client.modules ?? []).length,
                    visitsDone: visits.filter((visit) =>
                      isCompletedClientVisit(visit.status),
                    ).length,
                    visitsTotal: totalVisits,
                  })}
              ...
              {tab === "planejamento" ? (
                <div
                  id="client-detail-planning-panel"
                  role="tabpanel"
                  aria-labelledby={`${planningTab}-tab`}
                  tabIndex={-1}
                  className="space-y-4"
                >
                  {planningTab === "estrategico" ? (
              ...
              {activeArea === "area-dados" ? (
                <div
                  id="client-detail-data-panel"
                  role="tabpanel"
                  aria-labelledby={`${activeDataAreaTab}-tab`}
                  tabIndex={-1}
                >
                  {tab === "dados" ? (
                    <ClientDadosTab
              ...
              {tab === "operacao" ? (
                <div
                  id="client-detail-configuration-panel"
                  role="tabpanel"
                  aria-labelledby={`${operationTab}-tab`}
                  tabIndex={-1}
                  className="space-y-4"
                >
                  {operationTab === "modulos" ? (
            ...
            <StoreFormModal
              ...
              clientId={client.id}
              defaultType={
                storeModal.initial?.store_type === "matriz"
                  ? "matriz"
                  : "filial"
              }
            ...
            <DonoMasterPickerModal
              ...
              donos={persons
                .filter((person) => person.papeis.includes("DONO"))
                .map((person) => ({
                  id: person.id,
                  nome: person.nome,
                  email: person.email,
