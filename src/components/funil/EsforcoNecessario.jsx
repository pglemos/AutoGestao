...
function AlavancaItem({ label, valor }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-border-subtle last:border-0">
      ...
      <span className="text-[18px] font-bold text-mx-navy tabular-nums">{valor}</span>
    </div>
  );
}
...
export default function EsforcoNecessario({ funis, faltam, funisBase90, usou90 }) {
  ...
  const COR_CANAL = { Carteira: "bg-brand-primary-subtle border-brand-primary/30", Internet: "bg-status-info-surface border-status-info/30", Showroom: "bg-status-warning-surface border-status-warning/30" };
      ...
      {!canalPrincipal ? (
        ...
        <>
          ...
          <div className={`rounded-xl border p-4 mb-4 ${corPrincipal}`}>
            <p className="text-caption font-bold text-muted-foreground uppercase tracking-wide mb-1">
              Sua melhor base hoje é <span className="font-bold text-mx-navy">{canalPrincipal}</span>
            </p>
