...
export default function PeriodFilter({ value, onChange, customStart, customEnd, onCustomStart, onCustomEnd, dark }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className={`absolute right-0 mt-2 w-56 rounded-xl border shadow-xl z-[var(--mx-z-overlay)] py-2 ${dark ? "bg-mx-navy border-white/10" : "bg-white border-border-subtle"}`}>
          {OPTIONS.map(opt => (
            <button
              key={opt.value}
