    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {MONTHS.map((month, index) => <div key={month}><Label className="mb-1 block text-xs">{MONTHS_FULL[index]}</Label><Input value={values[index]} onChange={event => change(index, event.target.value)} className={`h-9 ${errors[index] ? 'border-status-error' : values[index] !== originalValues[index] ? 'border-amber-400 bg-status-warning-surface' : ''}`} />{errors[index] ? <p className="mt-0.5 text-xs text-status-error">{errors[index]}</p> : null}</div>)}
        </div>
