  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="rounded-lg border bg-slate-50 p-3"><div className="flex justify-between"><div><p className="text-xs text-muted-foreground">{getConsolidatedLabel(indicator.aggregationMode, SELECTED_MONTH_INDEX)}</p><p className="text-lg font-bold">{formatCellValue(consolidated, indicator.displayFormat, indicator.decimalPlaces)}</p></div><div className="text-right"><p className="text-xs text-muted-foreground">Formato</p><p className="text-sm font-medium">{FORMAT_LABELS[indicator.displayFormat]}</p></div></div>{changedCount ? <p className="mt-2 text-xs font-medium text-amber-600">{changedCount} mês(es) alterado(s)</p> : null}</div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {MONTHS.map((month, index) => <div key={month}><Label className="mb-1 block text-xs">{MONTHS_FULL[index]}</Label><Input value={values[index]} onChange={event => change(index, event.target.value)} className={`h-9 ${errors[index] ? 'border-red-500' : values[index] !== originalValues[index] ? 'border-amber-400 bg-amber-50' : ''}`} />{errors[index] ? <p className="mt-0.5 text-xs text-red-500">{errors[index]}</p> : null}</div>)}
        </div>
