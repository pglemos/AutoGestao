    ...
    return (
      <div className={cn("space-y-mx-xs w-full", className)}>
        <label htmlFor={fieldId} className="block ml-2">
          <Typography variant="label" tone="muted">{label}</Typography>
        </label>
        <div className="relative group">
          {icon && (
            <div className="absolute left-mx-sm top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-status-success-text transition-colors" aria-hidden="true">
              {icon}
            </div>
          )}
        <Input
