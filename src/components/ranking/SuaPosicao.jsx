...
export default function SuaPosicao({ posicao, total, atingimento, faltamValor, isVolume }) {
  ...
  return (
    <div className="relative bg-white rounded-2xl border border-border shadow-sm p-5 min-w-[220px] overflow-hidden flex flex-col justify-between">
      {/* bg trophy watermark */}
      <Trophy className="absolute right-2 top-2 opacity-[0.06]" style={{ width: 120, height: 120, color: chartTokens.success() }} />
      <div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        ...
        <div className="bg-status-info-surface border border-status-info/20 rounded-xl p-3 flex flex-col items-center gap-1 text-center">
          ...
          <p className="text-[16px] font-bold text-status-info-text leading-tight">{formatFaltam(faltamValor, isVolume)}</p>
