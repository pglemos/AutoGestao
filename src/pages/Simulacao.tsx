...
export default function Simulacao() {
  ...
  return (
    <MxModulePage id="simulation-selector">
      <MxModuleHeader eyebrow="Simulação" title="Simulação de Perfis" description="Valide as experiências reais de Vendedor, Gerente e Dono sem substituir sua identidade administrativa." actions={isSimulating ? <Button variant="secondary" onClick={() => { stopSimulation(); navigate('/simulacao', { replace: true }) }}>Encerrar simulação</Button> : undefined} />
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-3" aria-label="Perfis disponíveis para simulação">
        {OPTIONS.map(item => { const Icon = item.icon; return <MxSectionCard key={item.role} className="flex flex-col p-5"><span className="grid h-12 w-12 place-items-center rounded-xl bg-status-success-surface text-status-success-text"><Icon size={24} /></span><h2 className="mt-4 text-lg font-semibold text-foreground">{item.title}</h2><p className="mt-2 flex-1 text-sm leading-6 text-muted-foreground">{item.description}</p><Button className="mt-5" onClick={() => navigate(`/simulacao/${item.role}`)}><MonitorPlay size={18} />Simular {item.title}</Button></MxSectionCard> })}
      </section>
    </MxModulePage>
  )
}
